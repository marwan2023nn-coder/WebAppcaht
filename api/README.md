# Sofa API Documentation

This repository holds the API reference documentation for Sofa available at [https://developers.sofa.com/api-reference](https://developers.sofa.com/api-reference).

The Sofa API reference uses the [OpenAPI standard](https://openapis.org/) and the [ReDoc document generator](https://github.com/Rebilly/ReDoc).
...
APIs for [Playbooks](https://github.com/sofa/sofa-plugin-playbooks) are retrieved from GitHub at build time and integrated into the final YAML file.
...
To build the full YAML, run `make build` and it will be output to `v4/html/static/sofa-openapi-v4.yaml`. This will also check syntax using [swagger-cli](https://github.com/APIDevTools/swagger-cli).
...
Deployment is handled automatically by our Github Actions. When a pull request is merged, it will automatically be deployed to [https://developers.sofa.com/api-reference](https://developers.sofa.com/api-reference).
