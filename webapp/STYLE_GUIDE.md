...
- **React Bootstrap**: Avoid using React Bootstrap directly. Use `GenericModal` from `@workspace/components` instead of React Bootstrap's `Modal`.
...
- **Icons**: Prefer using icon components from `@workspace/compass-icons` for icons.
...
- **Organizing Redux State**: When adding new fields to the Redux state, put data that comes from the server in `workspace-redux` and in `state.entities`, and put web app state that needs to be persisted outside of `workspace-redux` and in `state.views`.
