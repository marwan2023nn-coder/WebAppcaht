...
This folder contains the client code for the Workspace web app. It's broken up into multiple packages each of which either contains an area of the app (such as `playbooks`) or shared logic used across other packages (such as the packages located in the `platform` directory). For anyone who's used to working in [the workspace/workspace-webapp repo](https://github.com/workspace/workspace-webapp), most of that is now located in `channels`.
...
- [Developer setup](https://developers.workspace.com/contribute/developer-setup/), now included with the Workspace server developer setup
- [Web app developer documentation](https://developers.workspace.com/contribute/more-info/webapp/)
