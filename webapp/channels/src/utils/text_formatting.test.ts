...
describe('linkOnlyMarkdown', () => {
    ...
    test('link without a title', () => {
        const text = 'Do you like https://www.workspace.com?';
        const output = formatText(text, options, emptyEmojiMap);
        expect(output).toBe(
            'Do you like <a class="theme markdown__link" href="https://www.workspace.com" target="_blank">' +
            'https://www.workspace.com</a>?');
    });
    test('link with a title', () => {
        const text = 'Do you like [Workspace](https://www.workspace.com)?';
        const output = formatText(text, options, emptyEmojiMap);
        expect(output).toBe(
            'Do you like <a class="theme markdown__link" href="https://www.workspace.com" target="_blank">' +
            'Workspace</a>?');
...
describe('highlightCurrentMentions', () => {
    ...
    const mentionKeys = [
        ...
        {key: 'Workspace'}, // Latin word
    ];

    it('should find and match Korean, Japanese, latin and Russian words', () => {
        const text = '메터모스트, notinkeys, マッターモスト, маттермост!, Workspace, notinkeys';
