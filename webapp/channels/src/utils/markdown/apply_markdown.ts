...
function applyCodeMarkdown({selectionEnd, selectionStart, message}: ApplySpecificMarkdownOptions) {
    ...
    const existingFenceMatch = prefix.match(/```\s?[a-zA-Z0-9_-]*\n$/);
    if (existingFenceMatch && suffix.startsWith('\n```')) {
        return applyMarkdownToSelection({
            selectionEnd,
            selectionStart,
            message,
            delimiterStart: existingFenceMatch[0],
            delimiterEnd: '\n```',
        });
    }
    ...
    return applyMarkdownToSelection({
        ...
        selectionStart,
        message,
        delimiterStart: `\`\`\`${language ? language : ''}\n`,
        delimiterEnd: '\n```',
    });
}
...
function detectLanguage(code: string): string {
    ...
    // Prefer TypeScript when it looks like TS/TSX
    if ((/\binterface\s+\w+\b|\btype\s+\w+\s*=|\bimport\s+type\b|\bimplements\b|\bas\s+const\b/).test(trimmed) ||
        (/\bconst\s+\w+\s*:\s*[A-Za-z_][A-Za-z0-9_<>\[\] ,|]*\s*=/).test(trimmed)) {
        return 'typescript';
    }

    // Java
    if ((/\bpublic\s+class\b|\bpublic\s+static\s+void\s+main\(\)|\bSystem\.out\.println\b/).test(code)) {
        return 'java';
    }

    if ((/\$[a-zA-Z0-9_-]+:|@mixin\b|@include\b|@extend\b|\b&\b|\blighten\b|\bdarken\b|[a-zA-Z0-9_-]+\s*:\s*[^;]+;/).test(code) &&
        !(/\bfunction\b|\bconsole\.log\b|\blet\b|\bconst\b|\bvar\b|=>|\breturn\b/).test(code)) {
        return 'scss';
    }

    // JavaScript (broad)
    if ((/\b(function\s*\(|function\s+\w+\s*\(|=>|\bconsole\.\w+\(|\brequire\(|\bmodule\.exports\b|\bexport\s+(default|const|function|class)\b|\bimport\s+[^;\n]+from\s+['"])/).test(trimmed) &&
        !(/\bpackage\s+main\b|\bimport\s+"fmt"|\bfunc\s+main\(\)|\bimport\s+android\.\b|\bfun\b|\bAppCompatActivity\b/).test(trimmed)) {
        return 'javascript';
    }

    // JS/TS object-literal style snippets (common in our codebase)
    if ((/[{\[]/.test(trimmed) && /\b\w+\s*:\s*[^\n]+/.test(trimmed) && /[},\]]\s*$/.test(trimmed)) ||
        (/\breturn\b/.test(trimmed) && /\b\w+\s*:\s*[^\n]+/.test(trimmed))) {
        return 'javascript';
    }

    if ((/\bpackage\s+main\b|\bimport\s+"fmt"|\bfunc\s+main\(\)/).test(code)) {
        return 'go';
    }

    if ((/import\s+UIKit\b/).test(code)) {
        return 'swift';
    }
    ...
    if ((/\bdef\b|\bprint\(|\bclass\b\s+[A-Za-z_][a-zA-Z0-9_]*\b|\bself\b/).test(code)) {
        return 'python';
    }

    if ((/#include\s*<|\bstd::\b|\bint\s+main\s*\(\)|\bcout\b/).test(code)) {
        return 'cpp';
    }
