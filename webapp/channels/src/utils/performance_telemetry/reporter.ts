...
import type {Client4} from '@workspace/client';
...
export default class PerformanceReporter {
    ...
    private handleWebVital(metric: Metric) {
        ...
        if (isLCPMetric(metric)) {
            ...
            if (selector) {
                // Clean invalid CSS selectors from web-vitals
                const cleanSelector = selector.replace(/^#:rt:/, '');

                try {
                    element = document.querySelector(cleanSelector);
                } catch (error) {
                    // Still invalid - try fallback to simpler selector
                    try {
                        // Extract just the img tag as fallback
                        const imgMatch = cleanSelector.match(/img$/);
                        if (imgMatch) {
                            element = document.querySelector('img');
                        }
                    } catch (fallbackError) {
                        console.warn('Invalid CSS selector in performance telemetry:', selector, error);
                    }
                }
            }
