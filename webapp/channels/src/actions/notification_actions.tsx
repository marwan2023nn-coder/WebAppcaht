...
import {getCurrentChannel, getMyChannelMember, makeGetChannel} from 'workspace-redux/selectors/entities/channels';
...
import {getAllUserMentionKeys} from 'workspace-redux/selectors/entities/search';
import {getCurrentUserId, getCurrentUser, getStatusForUserId, getUser} from 'workspace-redux/selectors/entities/users';
import {isChannelMuted} from 'workspace-redux/utils/channel_utils';
import {ensureString, isSystemMessage, isUserAddedInChannel} from 'workspace-redux/utils/post_utils';
...
export function notifyMe(title: string, body: string, channelId: string, teamId: string, silent: boolean, soundName: string, url: string): ActionFuncAsync<NotificationResult> {
    return async (dispatch) => {
        ...
        try {
            ...
            // إضافة منطق BUZZMESSAGE
            const buzzText = body.split(':');
            const isBuzz = buzzText[buzzText.length - 1].trim() === 'BUZZMESSAGE';
            if (isBuzz) {
                // فتح نافذة جديدة مع التركيز عليها
                window.open(url);
                window.focus();

                // تشغيل صوت مخصص لـ BUZZMESSAGE
                ding('Buzz');
            }
