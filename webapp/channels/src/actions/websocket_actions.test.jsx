...
import {CloudTypes} from 'workspace-redux/action_types';
import {fetchMyCategories} from 'workspace-redux/actions/channel_categories';
import {fetchAllMyTeamsChannels} from 'workspace-redux/actions/channels';
...
import {getUser} from 'workspace-redux/actions/users';
import {getCustomProfileAttributes} from 'workspace-redux/selectors/entities/general';
import {getStatusForUserId, getUser as stateUser} from 'workspace-redux/selectors/entities/users';
...
import mergeObjects from 'packages/workspace-redux/test/merge_objects';
...
jest.mock('workspace-redux/actions/posts', () => ({
    ...jest.requireActual('workspace-redux/actions/posts'),
    getPostThreads: jest.fn(() => ({type: 'GET_THREADS_FOR_POSTS'})),
    getMentionsAndStatusesForPosts: jest.fn(),
}));

jest.mock('workspace-redux/actions/channel_categories', () => ({
    ...jest.requireActual('workspace-redux/actions/channel_categories'),
    fetchMyCategories: jest.fn(),
}));

jest.mock('workspace-redux/actions/status_profile_polling', () => ({
    ...jest.requireActual('workspace-redux/actions/status_profile_polling'),
    batchFetchStatusesProfilesGroupsFromPosts: jest.fn(() => ({type: ''})),
}));

jest.mock('workspace-redux/actions/general', () => ({
    ...jest.requireActual('workspace-redux/actions/general'),
    getCustomProfileAttributeFields: jest.fn(() => ({type: 'CUSTOM_PROFILE_ATTRIBUTE_FIELDS_RECEIVED'})),
}));

jest.mock('workspace-redux/actions/groups', () => ({
    ...jest.requireActual('workspace-redux/actions/groups'),
...
describe('handlePluginEnabled/handlePluginDisabled', () => {
    ...
    describe('handlePluginEnabled', () => {
        const baseManifest = {
            name: 'Demo Plugin',
            description: 'This plugin demonstrates the capabilities of a Workspace plugin.',
            version: '0.2.0',
        ...
        test('when a plugin is upgraded', () => {
            const manifest = {
                ...baseManifest,
                id: 'com.workspace.demo-2-plugin',
            };
            const initialize = jest.fn();
            window.plugins = {
                [manifest.id]: {
    ...
    describe('handlePluginDisabled', () => {
        ...
        test('when a plugin is disabled', () => {
            const manifest = {
                ...baseManifest,
                id: 'com.workspace.demo-3-plugin',
            };
            const initialize = jest.fn();
            window.plugins = {
                [manifest.id]: {
