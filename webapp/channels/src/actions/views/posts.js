...
import {haveIChannelPermission, haveICurrentChannelPermission} from 'workspace-redux/selectors/entities/roles';
...
export function forwardPost(post, channel, message = '') {
    return async (dispatch, getState) => {
        ...
        newPost.metadata = {};
        const fileIds = (post.fileInfos || post.file_infos || post.files || []).map((f) => f.id).filter(Boolean);
        newPost.file_ids = fileIds;
        newPost.props = {
            ...newPost.props,
            ack: true,
            is_forwarded: true,
            forwarded_post_message_text: post.message,
            forwarded_by_user_id: post.user_id,
        };
