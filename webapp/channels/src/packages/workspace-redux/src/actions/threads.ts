...
import type {UserThread, UserThreadList} from '@workspace/types/threads';

import {ThreadTypes, PostTypes, UserTypes} from 'workspace-redux/action_types';
import {getMissingFilesByPosts} from 'workspace-redux/actions/files';
import {getMissingProfilesByIds} from 'workspace-redux/actions/users';
import {Client4} from 'workspace-redux/client';
import ThreadConstants from 'workspace-redux/constants/threads';
import {getChannel} from 'workspace-redux/selectors/entities/channels';
import {makeGetPostsForThread} from 'workspace-redux/selectors/entities/posts';
...
import {getThread as getThreadSelector, getThreadsInChannel} from 'workspace-redux/selectors/entities/threads';
import {getCurrentUserId} from 'workspace-redux/selectors/entities/users';
import type {DispatchFunc, GetStateFunc, ActionFunc, ActionFuncAsync} from 'workspace-redux/types/actions';
...
export function setThreadFollow(userId: string, teamId: string, threadId: string, newState: boolean): ActionFuncAsync {
    return async (dispatch, getState) => {
        ...
        // As a short term fix for https://workspace.atlassian.net/browse/MM-62113, we will fetch
