...
import type { AccessControlPolicy } from '@workspace/types/access_control';
import type { ClusterInfo, AnalyticsRow, AnalyticsState, AdminState } from '@workspace/types/admin';
import type { Audit } from '@workspace/types/audits';
import type { Compliance } from '@workspace/types/compliance';
import type { AdminConfig, EnvironmentConfig } from '@workspace/types/config';
import type { DataRetentionCustomPolicy } from '@workspace/types/data_retention';
import type { MixedUnlinkedGroupRedux } from '@workspace/types/groups';
import type { PluginRedux, PluginStatusRedux } from '@workspace/types/plugins';
import type { SamlCertificateStatus, SamlMetadataResponse } from '@workspace/types/saml';
import type { UserAccessToken, UserProfile } from '@workspace/types/users';
import type { RelationOneToOne, IDMappedObjects } from '@workspace/types/utilities';

import type { MMReduxAction } from 'workspace-redux/action_types';
import { AdminTypes, UserTypes } from 'workspace-redux/action_types';
import { Stats } from 'workspace-redux/constants';
...
function dataRetentionCustomPoliciesCount(state = 0, action: MMReduxAction) {
    switch (action.type) {
        case AdminTypes.RECEIVED_DATA_RETENTION_CUSTOM_POLICIES:
            return action.data?.total_count || 0;
        case UserTypes.LOGOUT_SUCCESS:
            return 0;
        default:
            return state;
