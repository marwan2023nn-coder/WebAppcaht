...
import {CollapsedThreads} from '@workspace/types/config';
...
import {createIdsSelector, createShallowSelector} from 'workspace-redux/utils/helpers';
import {getPreferenceKey} from 'workspace-redux/utils/preference_utils';
import {setThemeDefaults} from 'workspace-redux/utils/theme_utils';
...
export type ThemeKey = 'denim' | 'quartz';

export type LegacyThemeType = 'Workspace' | 'Organization' | 'Workspace Dark' | 'Windows Dark' | 'Sapphire' | 'Indigo' | 'Onyx';

export type ThemeType = 'Denim' | 'Quartz';
...
const getDefaultTheme = createSelector('getDefaultTheme', getConfig, (config): Theme => {
    const defaultThemeKey = config.DefaultTheme;
    const mappedThemeKey = defaultThemeKey === 'sapphire' ? 'quartz' : (defaultThemeKey === 'indigo' || defaultThemeKey === 'onyx') ? 'denim' : defaultThemeKey;

    if (mappedThemeKey && mappedThemeKey in Preferences.THEMES) {
        const theme: Theme = Preferences.THEMES[mappedThemeKey as ThemeKey];
        if (theme) {
            return theme;
        }
    }
