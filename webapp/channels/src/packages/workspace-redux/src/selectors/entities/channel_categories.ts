...
import type {ChannelCategory, ChannelCategoryType} from '@workspace/types/channel_categories';
import {CategorySorting} from '@workspace/types/channel_categories';
import type {Channel, ChannelMembership, ChannelMessageCount} from '@workspace/types/channels';
import type {GlobalState} from '@workspace/types/store';
import type {UserProfile} from '@workspace/types/users';
import type {IDMappedObjects, RelationOneToOne} from '@workspace/types/utilities';

import {General, Preferences} from 'workspace-redux/constants';
...
import {getChannelMessageCounts, getCurrentChannelId, getMyChannelMemberships, makeGetChannelsForIds} from 'workspace-redux/selectors/entities/channels';
import {getCurrentUserLocale} from 'workspace-redux/selectors/entities/i18n';
import {get as getPreference, getMyPreferences, getTeammateNameDisplaySetting, getVisibleDmGmLimit, isCollapsedThreadsEnabled} from 'workspace-redux/selectors/entities/preferences';
import {getCurrentUserId, getUserIdsInChannels, getUserStatuses} from 'workspace-redux/selectors/entities/users';
import {
    calculateUnreadCount,
    getUserIdFromChannelName,
    isChannelMuted,
} from 'workspace-redux/utils/channel_utils';
...
// makeFilterDmGmByStatusPreference filters DM/GM channels by teammate status based on a user preference.
// Preference key: sidebar_settings -- dm_gm_status_filter with values: 'all' | 'online' | 'away' | 'offline'.
export function makeFilterDmGmByStatusPreference(): (state: GlobalState, channels: Channel[], categoryType: string) => Channel[] {
    return createSelector(
        'makeFilterDmGmByStatusPreference',
        (state: GlobalState, channels: Channel[]) => channels,
        (state: GlobalState, channels: Channel[], categoryType: string) => categoryType,
        getCurrentUserId,
        getUserStatuses,
        getUserIdsInChannels,
        (state: GlobalState) => getPreference(state, Preferences.CATEGORY_SIDEBAR_SETTINGS, 'dm_gm_status_filter', 'all'),
        (channels, categoryType, currentUserId, userStatuses, usersInChannels, statusFilter) => {
            ...
            const filtered = channels.filter((channel) => {
                if (channel.type === General.DM_CHANNEL) {
                    ...
                    const status = userStatuses[teammateId];
                    return matches(status);
                }

                if (channel.type === General.GM_CHANNEL) {
                    const memberIds = usersInChannels[channel.id];
                    if (!memberIds) {
                        return true;
                    }
                    for (const userId of memberIds) {
                        ...
                        if (matches(userStatuses[userId])) {
                            return true;
                        }
                    }
                    return false;
...
export function makeFilterAutoclosedDMs(): (state: GlobalState, channels: Channel[], categoryType: string) => Channel[] {
    return createSelector(
        ...
        (channels, categoryType, currentChannelId, profiles, currentUserId, myMembers, messageCounts, limitPref, myPreferences, collapsedThreads) => {
            ...
            if (limitPref !== Preferences.HIGHEST_DM_SHOW_COUNT) {
...
export function makeFilterAndSortChannelsForCategory() {
    ...
    const filterDmGmByStatusPreference = makeFilterDmGmByStatusPreference();
    ...
    return (state: GlobalState, originalChannels: Channel[], category: ChannelCategory) => {
        ...
        channels = filterDmGmByStatusPreference(state, channels, category.type);
