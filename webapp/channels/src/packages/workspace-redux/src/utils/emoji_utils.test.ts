...
import * as EmojiUtils from 'workspace-redux/utils/emoji_utils';
...
describe('EmojiUtils', () => {
    ...
    describe('getEmojiImageUrl', () => {
        ...
        test('return correct url for workspace emoji', () => {
            expect(EmojiUtils.getEmojiImageUrl(TestHelper.getCustomEmojiMock({id: 'workspace', category: 'custom'}))).toBe('/static/emoji/workspace.png');

            expect(EmojiUtils.getEmojiImageUrl(TestHelper.getCustomEmojiMock({id: 'workspace'}))).toBe('/static/emoji/workspace.png');
        });
