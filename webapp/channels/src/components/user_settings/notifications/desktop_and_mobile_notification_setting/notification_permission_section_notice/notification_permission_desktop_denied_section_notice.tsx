...
export default function NotificationPermissionDesktopDeniedSectionNotice(props: Props) {
    ...
    const handleInstructionButtonClick = useCallback(() => {
        window.open('https://workspace.com/pl/manage-notifications', '_blank', 'noopener,noreferrer');
    }, []);
    ...
    const text = checkedPermissionDenied ? intl.formatMessage({
        ...
        defaultMessage: 'Notifications for this Workspace server are blocked. To receive notifications, please enable them manually.',
    }) : intl.formatMessage({
        id: 'user.settings.notifications.desktopAndMobile.notificationSection.permissionDeniedDesktop.message',
        defaultMessage: "You're missing important message and call notifications from Workspace. To start receiving them, please enable them manually.",
    });
