...
import type {UserProfile, UserTimezone} from '@workspace/types/users';
...
function getDisplayStateFromProps(props: Props) {
    return {
        ...
        messageDisplay: Preferences.MESSAGE_DISPLAY_CLEAN,
...
export default class UserSettingsDisplay extends React.PureComponent<Props, State> {
    ...
    handlemessageDisplayRadio(messageDisplay: string) {
        this.setState({messageDisplay: Preferences.MESSAGE_DISPLAY_CLEAN});
    }
    ...
    render() {
        ...
        const languageInfo = getLanguageInfo(userLocale);
        const localeName = (languageInfo && typeof languageInfo.name === 'string') ? languageInfo.name : 'العربية';
