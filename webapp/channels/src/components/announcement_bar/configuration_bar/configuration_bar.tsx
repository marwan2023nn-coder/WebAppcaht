...
import type {ClientConfig, WarnMetricStatus} from '@workspace/types/config';
        ...
        if (isTrialLicense(props.license) && typeof daysUntilLicenseExpires !== 'undefined' && daysUntilLicenseExpires <= 14 && !props.dismissedExpiringTrialLicense) {
            ...
            if (daysUntilLicenseExpires < 1) {
                const viewportBasedMessage = width < Constants.MOBILE_SCREEN_WIDTH ? formatMessage({
                    ...
                    defaultMessage: 'This is the last day of your free trial. Purchase a license now to continue using Workspace Professional and Enterprise features.',
                });
                message = (
                    <>
                        <img
    ...
    if (props.canViewSystemErrors && props.config?.SiteURL === '') {
        ...
        if (props.config?.EnableSignUpWithGitLab === 'true') {
            siteURLMessage = formatMessage({
                ...
                defaultMessage: 'Please configure your <linkSite>site URL</linkSite> either on the <linkConsole>System Console</linkConsole> or, if you\'re using GitLab Workspace, in gitlab.rb.',
            }, values);
        } else {
            siteURLMessage = formatMessage({
