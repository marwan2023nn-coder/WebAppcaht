...
const DesktopAuthToken: React.FC<Props> = ({href, onLogin}: Props) => {
    ...
    if (status === DesktopAuthStatus.LoggedIn) {
        ...
        subMessage = (
            <FormattedMessage
                id='desktop_auth_token.complete.openworkspace'
                defaultMessage='Click on <b>Open workspace</b> in the browser prompt to <a>launch the desktop app</a>'
                values={{
        ...
        bottomMessage = (
            <FormattedMessage
                ...
                defaultMessage='Having trouble logging in? <a>Open workspace in your browser</a>'
                values={{
                    a: (chunks: React.ReactNode) => {
                        return (
