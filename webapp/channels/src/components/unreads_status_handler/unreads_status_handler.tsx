...
import {basicUnreadMeta} from 'workspace-redux/selectors/entities/channels';
import type {BasicUnreadStatus} from 'workspace-redux/selectors/entities/channels';
...
export class UnreadsStatusHandlerClass extends React.PureComponent<Props> {
    ...
    updateTitle = () => {
        ...
        if (currentChannel && currentTeam && currentChannel.id) {
            ...
            document.title = formatMessage({id: 'sidebar.team_select', defaultMessage: '{siteName} - Join a team'}, {siteName: currentSiteName || 'Workspace'});
        }
    };
