...
describe('components/AboutBuildModal', () => {
    ...
    test('should match snapshot for enterprise edition', () => {
        ...
        expect(screen.getByRole('link', {name: 'workspace.com'})).toHaveAttribute('href', 'https://workspace.com/?utm_source=workspace&utm_medium=in-product&utm_content=about_build_modal&uid=&sid=&edition=enterprise&server_version=3.6.0');
        ...
        expect(screen.getByRole('link', {name: 'server'})).toHaveAttribute('href', 'https://github.com/workspace/workspace-server/blob/master/NOTICE.txt');
        expect(screen.getByRole('link', {name: 'desktop'})).toHaveAttribute('href', 'https://github.com/workspace/desktop/blob/master/NOTICE.txt');
        expect(screen.getByRole('link', {name: 'mobile'})).toHaveAttribute('href', 'https://github.com/workspace/workspace-mobile/blob/master/NOTICE.txt');
    });
    ...
    test('should match snapshot for team edition', () => {
        ...
        expect(screen.getByTestId('aboutModalVersionInfo')).toHaveTextContent('Database Schema Version: 77');
        expect(screen.getByTestId('aboutModalVersionInfo')).toHaveTextContent('Build Number: 123456');
        expect(screen.getByText('Workspace Team Edition')).toBeInTheDocument();
        ...
        expect(screen.getByRole('link', {name: 'workspace.com/community/'})).toHaveAttribute('href', 'https://workspace.com/community/?utm_source=workspace&utm_medium=in-product&utm_content=about_build_modal&uid=&sid=&edition=team&server_version=3.6.0');
    ...
    test('should match snapshot for cloud edition', () => {
        ...
        expect(screen.getByText('Workspace Cloud')).toBeInTheDocument();
        expect(screen.getByText('High trust messaging for the enterprise')).toBeInTheDocument();
        expect(screen.getByTestId('aboutModalVersion')).toHaveTextContent('Workspace Version: 3.6.0');
    ...
    test('should show n/a if this is a dev build', () => {
        ...
        expect(screen.getByText('Workspace Team Edition')).toBeInTheDocument();
        expect(screen.getByText('All your team communication in one place, instantly searchable and accessible anywhere.')).toBeInTheDocument();
        expect(screen.getByRole('link', {name: 'workspace.com/community/'})).toHaveAttribute('href', 'https://workspace.com/community/?utm_source=workspace&utm_medium=in-product&utm_content=about_build_modal&uid=&sid=&edition=team&server_version=dev');
        expect(screen.queryByText('EE Build Hash: 0123456789abcdef')).not.toBeInTheDocument();
    ...
    test('should show default tos and privacy policy links and not the config links', () => {
        ...
        expect(screen.getByRole('link', {name: 'Terms of Use'})).toHaveAttribute('href', `${AboutLinks.TERMS_OF_SERVICE}?utm_source=workspace&utm_medium=in-product&utm_content=about_build_modal&uid=currentUserId&sid=&edition=enterprise&server_version=3.6.0`);

        expect(screen.getByRole('link', {name: 'Privacy Policy'})).toHaveAttribute('href', `${AboutLinks.PRIVACY_POLICY}?utm_source=workspace&utm_medium=in-product&utm_content=about_build_modal&uid=currentUserId&sid=&edition=enterprise&server_version=3.6.0`);
