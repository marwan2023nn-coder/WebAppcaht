...
import {loadMe} from 'workspace-redux/actions/users';
...
import {getTeamByName, getMyTeamMember} from 'workspace-redux/selectors/entities/teams';
...
const Login = () => {
    ...
    const getExternalLoginOptions = () => {
        ...
        if (enableSignUpWithOpenId) {
            ...
            externalLoginOptions.push({
                ...
                style: {color: OpenIdButtonColor, borderColor: OpenIdButtonColor, background: 'transparent'},
            ...
            if (isEmbedded()) {
                ...
                const messagePromise = new Promise<boolean>((resolve) => {
                    ...
                    const messageHandler = (event: MessageEvent) => {
                        ...
                        if (event.data && event.data.type === 'Workspace_external_auth_login' && event.data.ack === true) {
                ...
                window.parent.postMessage({
                    type: 'Workspace_external_auth_login',
                    provider,
                    href,
    ...
    const getCardTitle = () => {
        ...
        } else if (!enableBaseLogin && enableExternalSignup) {
            title = formatMessage({id: 'login.cardtitle.external', defaultMessage: 'Log in with one of the following:'});
        } else {
            title = formatMessage({id: 'login.cardtitle', defaultMessage: 'Log in'});
        }

        return (
            <>
                <span>{title}</span>
                <span className='Log-Text'> {formatMessage({id: t('login.welcome_message'), defaultMessage: 'Log in to start chatting with your team.'})}</span>
            </>
        );
    };
                            ...
                            {enableBaseLogin && (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        />
                                        <h5>{formatMessage({id: 'user.settings.security.password', defaultMessage: 'Password'})} </h5>
                                        <>
                                            <PasswordInput
                                                ref={passwordInput}
                                ...
                                >
                                    {getExternalLoginOptions().map((option) => (
                                        <ExternalLoginButton
                                            key={option.id}
                                            direction={
                                                enableBaseLogin ? undefined : 'column'
                                            }
                                            {...option}
                                        />
                                    ))}
