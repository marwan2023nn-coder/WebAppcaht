...
import { MdHome, MdMoreHoriz, MdOutlineNavigateBefore, MdOutlineNavigateNext } from "react-icons/md";
...
const BreadCrumb = () => {
  ...
  const getBreadCrumbWidth = () => {
    ...
    const paddingInlineStart = parseFloat(containerStyles.paddingInlineStart);
    const moreBtnGap = hiddenFolders.length > 0 ? 1 : 0;
    const flexGap = parseFloat(containerStyles.gap) * (folders.length + moreBtnGap);
    return containerWidth - (paddingInlineStart + flexGap);
  };
      ...
      <div className="breadcrumb" ref={breadCrumbRef}>
        {folders.map((folder, index) => (
          <div key={index} style={{ display: "contents" }}>
            ...
            >
              {index === 0 ? <IoHomeOutline size={20}  style={{marginLeft:"10px"}} color="#808887" /> : <MdOutlineNavigateBefore size={30} color="#808887"  />}
             <span >
             {folder.name}
              </span>
            </span>
