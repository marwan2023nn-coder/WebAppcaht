...
import { getPost, makeGetCommentCountForPost, makeIsPostCommentMention, isPostAcknowledgementsEnabled, isPostPriorityEnabled, isPostFlagged } from 'workspace-redux/selectors/entities/posts';
import type { UserActivityPost } from 'workspace-redux/selectors/entities/posts';
import {
    get,
    getBool,
    isCollapsedThreadsEnabled,
} from 'workspace-redux/selectors/entities/preferences';
import { getCurrentTeam, getTeam, getTeamMemberships } from 'workspace-redux/selectors/entities/teams';
...
import {
    ...
    getBurnOnReadPostMaxExpiration,
} from 'selectors/burn_on_read_posts';
...
import { revealBurnOnReadPost, burnPostNow } from 'actions/burn_on_read_posts';
...
function makeMapStateToProps() {
    ...
    return function mapStateToProps(state: GlobalState, ownProps: OwnProps) {
        ...
        const forwardedByUserId = post.props?.forwarded_by_user_id as string;
        const forwardedByUser = forwardedByUserId ? getUser(state, forwardedByUserId) : undefined;
        const forwardedByUserFullName = forwardedByUser ? (getFullName(forwardedByUser) || forwardedByUser.username) : '';
        const forwardedByUserAvatar = forwardedByUserId ? imageURLForUser(forwardedByUserId, forwardedByUser?.last_picture_update) : '';
        ...
        return {
            ...
            forwardedByUserFullName,
            forwardedByUserAvatar,
            isBurnOnRead: isThisPostBurnOnReadPost(post),
            isRevealed: hasUserRevealedBurnOnReadPost(state, post.id),
            isConcealed: shouldDisplayConcealedPlaceholder(state, post.id),
            expireAt: getBurnOnReadPostExpiration(state, post.id),
            maxExpireAt: getBurnOnReadPostMaxExpiration(state, post.id),
        };
    };
}
