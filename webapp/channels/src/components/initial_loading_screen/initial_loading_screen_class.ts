...
const ANIMATION_CLASS_FOR_workspace_logo_HIDE = 'LoadingAnimation__compass-shrink';
...
export class InitialLoadingScreenClass {
    ...
    private init() {
        if (isDesktopApp()) {
            // Let Workspace desktop handle the loading screen
            this.destroy();
            return;
        }
    ...
    private handleAnimationEndEvent(event: AnimationEvent) {
        ...
        if (event.animationName === ANIMATION_CLASS_FOR_workspace_logo_HIDE || event.animationName === ANIMATION_CLASS_FOR_COMPLETE_LOADER_HIDE) {
            if (!this.isLoading) {
