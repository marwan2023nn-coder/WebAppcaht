...
describe('useExternalLink', () => {
    it('keep non workspace links untouched', () => {
    ...
    it('mailto links are untouched even if to workspace emails', () => {
        const mailtoURL = 'mailto:example@workspace.com?subject=123&body=456';
    ...
    it('all base queries are set correctly', () => {
        const url = 'https://www.workspace.com/some/url';
        ...
        expect(parsedLink.searchParams.get('utm_source')).toBe('workspace');
        ...
        expect(queryParams.utm_source).toBe('workspace');
    ...
    it('keep existing query parameters untouched', () => {
        const url = 'https://www.workspace.com/some/url?myParameter=true';
        const {result: {current: [href, queryParams]}} = renderHookWithContext(() => useExternalLink(url), getBaseState());
        const parsedLink = new URL(href);
    ...
    it('keep anchors untouched', () => {
        const url = 'https://www.workspace.com/some/url?myParameter=true#myAnchor';
    ...
    it('existing params gets preference over default and overwritten params', () => {
        ...
        const url = `https://www.workspace.com/some/url?utm_content=${expectedContent}&utm_source=${expectedSource}`;
    ...
    it('do not substitute %20 on query params', () => {
        const url = 'https://www.workspace.com/some/url?subject=hello%20world';
        const {result: {current: [href]}} = renderHookWithContext(() => useExternalLink(url), getBaseState());
