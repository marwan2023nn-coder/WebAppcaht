...
import Modal from '.././channel_header/ai_model/components/Modal'; // Adjust the import based on your project structure
// eslint-disable-next-line import/order
import Model from '.././channel_header/ai_model/model';
...
export default function FileAttachment(props: Props) {
   ...
   const onAttachmentClick = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
        if (props.fileInfo.archived) {
            return; // If the file is archived, do nothing
        }

        e.preventDefault();
        if ('blur' in e.target) {
            (e.target as HTMLElement).blur(); // Remove focus from the clicked element
        }

        // Use onAttachmentClickss to open the URL
        // onAttachmentClickss(props.fileInfo.id, props.fileInfo.extension);

        if (props.handleImageClick && props.fileInfo.extension.toLowerCase() !== 'zip' && props.fileInfo.extension.toLowerCase() !== 'rar') {
    ...
    if (fileType === FileTypes.AUDIO && !props.fileInfo.archived) {
        return (
            <div className='post-image__column'>
                <AudioPlayer
                    audioUrl={getFileUrl(props.fileInfo.id)}
                    mimeType={mimeType}
                    senderId={props.fileInfo.user_id}
                />
            ...
            >
                ...
                <>
                    ...
                    >
                        <Model
                            fileId={props.fileInfo.id}
                            fileType={props.fileInfo.extension}
                        />
                    </Modal>
                </>
            </div>
