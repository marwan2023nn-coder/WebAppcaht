...
import {setSystemEmojis} from 'workspace-redux/actions/emojis';
import {setUrl} from 'workspace-redux/actions/general';
...
export default class Root extends React.PureComponent<Props, State> {
    ...
    private showLandingPageIfNecessary = () => {
        ...
        if (window.location.hostname?.endsWith('.test.workspace.com')) {
            return;
        }
...
export function doesRouteBelongToTeamControllerRoutes(pathname: RouteComponentProps['location']['pathname']): boolean {
    ...
    const TEAM_CONTROLLER_PATH_PATTERN = new RegExp(`^/(?!admin_console)([a-z0-9\\-_]+)/(channels|messages|threads|drafts|recaps|integrations|emoji|${SCHEDULED_POST_URL_SUFFIX})(/.*)?$`);
