...
export default class MoreDirectChannels extends React.PureComponent<Props, State> {
    ...
    handleSubmit = (values = this.state.values) => {
        ...
        const openPromise = userIds.length === 1 ?
            actions.openDirectChannelToUserId(userIds[0]) :
            actions.openGroupChannelToUserIds(userIds);

        openPromise.then((result) => {
            this.setState({saving: false});

            const channel = result?.data;
            if (channel && this.props.currentTeamName) {
