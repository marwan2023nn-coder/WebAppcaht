...
import {getFileDownloadUrl, getFilePreviewUrl, getFileUrl} from 'workspace-redux/utils/file_utils';
...
export type Props = {
    ...
    /**
     * Optional mapping of postId -> Post for fileInfos that belong to other posts.
     */
    postsById?: Record<string, Post | undefined>;
...
export default class FilePreviewModal extends React.PureComponent<Props, State> {
    ...
    render() {
        ...
        let postForCurrentFile: Post | undefined;
        if (isFileInfo(fileInfo) && this.props.postsById) {
            const postId = fileInfo.post_id;
            if (typeof postId === 'string' && postId.length > 0) {
                postForCurrentFile = this.props.postsById[postId];
            }
        }
        const effectivePost = postForCurrentFile || this.props.post;
        ...
        if (isFileInfo(fileInfo) && !fileInfo.archived) {
            for (const preview of this.props.pluginFilePreviewComponents) {
                if (preview.override(fileInfo, effectivePost)) {
                    content = (
                        <preview.component
                            fileInfo={fileInfo}
                            post={effectivePost}
        ...
        return (
            <Modal
                ...
                backdrop={false}
                role='none'
                style={{paddingInlineStart: 0,paddingLeft:'0'}}
                        ...
                        >
                            ...
                            >
                                <FilePreviewModalHeader
                                    ...
                                    post={(effectivePost || this.props.post)!}
