...
import {PaperclipIcon} from '@workspace/compass-icons/components';
import type {ServerError} from '@workspace/types/errors';
import type {FileInfo, FileUploadResponse} from '@workspace/types/files';
...
export class FileUpload extends PureComponent<Props, State> {
    ...
    checkPluginHooksAndUploadFilesWithClientIds = (files: FileList | File[], clientIds: string[]) => {
        // clear any existing errors
        this.props.onUploadError(null);

        let sortedFiles = Array.from(files).sort((a, b) => a.name.localeCompare(b.name, this.props.locale, {numeric: true}));

        let uploadedByHook = false;
        const uploadWithProvidedClientIds = (hookFiles: File[]) => {
            uploadedByHook = true;

            // If the hook changes the number of files, we can't reliably preserve ids.
            if (hookFiles.length !== clientIds.length) {
                this.uploadFiles(hookFiles);
                return;
            }

            this.uploadFilesWithClientIds(hookFiles, clientIds);
        };

        const willUploadHooks = this.props.pluginFilesWillUploadHooks;
        for (const h of willUploadHooks) {
            const result = h.hook?.(sortedFiles, uploadWithProvidedClientIds);
        ...
        if (clientIds.length !== sortedFiles.length) {
            // Fallback to default behavior if we can't align ids to files
            this.uploadFiles(sortedFiles);
            return;
        }

        this.uploadFilesWithClientIds(sortedFiles, clientIds);
    };

    uploadFilesWithClientIds = (files: File[], clientIds: string[]) => {
        ...
        for (let i = 0; i < files.length && numUploads < uploadsRemaining; i++) {
            ...
            if (files[i].size === 0) {
                zeroFiles.push(files[i]);
            }

            const clientId = clientIds[i];

            const handleSuccess = (data: FileUploadResponse, successChannelId: string, currentRootId: string) => {
                // Some servers/plugins may not preserve the provided client id.
                // Force the client id used for this request so the caller can reliably replace previews.
                const patched = {
                    ...data,
                    client_ids: [clientId],
                } as FileUploadResponse;
                this.fileUploadSuccess(patched, successChannelId, currentRootId);
            };
    ...
    render() {
        ...
        if (this.props.pluginFileUploadMethods.length === 0) {
            ...
            const pluginFileUploadMethods = this.props.pluginFileUploadMethods.filter((item) => item.pluginId !== 'voice').map((item) => {
                return (
                    <li
                                       ...
                                       <svg
                                        ...
                                        >
                                            <path
                                                fillRule='evenodd'
                                                clipRule='evenodd'
                                                d='M3.75 1H6C7.38071 1 8.5 2.11929 8.5 3.5V4.75C8.5 5.44036 9.05964 6 9.75 6H11C12.3807 6 13.5 7.11929 13.5 8.5V13.75C13.5 14.4404 12.9404 15 12.25 15H3.75C3.05964 15 2.5 14.4404 2.5 13.75V2.25C2.5 1.55964 3.05964 1 3.75 1ZM8.35355 7.64645C8.25979 7.55268 8.13261 7.5 8 7.5C7.86739 7.5 7.74021 7.55268 7.64645 7.64645L5.64645 9.64645C5.45118 9.84171 5.45118 10.1583 5.64645 10.3536C5.84171 10.5488 6.15829 10.5488 6.35355 10.3536L7.5 9.20711L7.5 12C7.5 12.2761 7.72386 12.5 8 12.5C8.27614 12.5 8.5 12.2761 8.5 12L8.5 9.20711L9.64645 10.3536C9.84171 10.5488 10.1583 10.5488 10.3536 10.3536C10.5488 10.1583 10.5488 9.84171 10.3536 9.64645L8.35355 7.64645Z'
                                                fill='#00987E'
                                            />
                                            <path
                                                d='M9.5 3.5C9.5 2.62463 9.17864 1.82432 8.64746 1.2106C10.9134 1.80252 12.6975 3.58658 13.2894 5.85254C12.6757 5.32136 11.8754 5 11 5H9.75C9.61193 5 9.5 4.88807 9.5 4.75V3.5Z'
                                                fill='#00987E'
                                            />
                                        </svg>
                                    </span>
