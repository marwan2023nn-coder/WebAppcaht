...
import {SyncIcon} from '@workspace/compass-icons/components';
import type {ServerError} from '@workspace/types/errors';
import type {UserPropertyField} from '@workspace/types/properties';
import type {Team, TeamMembership} from '@workspace/types/teams';
import type {UserProfile} from '@workspace/types/users';

import type {ActionResult} from 'workspace-redux/types/actions';
import {isEmail, getInputTypeFromValueType} from 'workspace-redux/utils/helpers';
        ...
        <ReactSelect
            ...
            styles={{
                ...
                control: (provided, state) => ({
                    ...provided,
                    borderColor: state.isFocused ? '#00987e' : provided.borderColor,
                    boxShadow: state.isFocused ? '0 0 0 1px #00987e' : provided.boxShadow,
                    ':hover': {
                        ...provided[':hover'],
                        borderColor: '#00987e',
                    },
                ...
                option: (provided, state) => {
                    const bg = (state.isFocused || state.isSelected) ? '#00987e' : '#ffffff';
                    const color = (state.isFocused || state.isSelected) ? '#ffffff' : '#1f2228';
                    return {
                        ...provided,
                        backgroundColor: bg,
                        color,
...
export class SystemUserDetail extends PureComponent<Props, State> {
    ...
    checkForChanges = () => {
        ...
        const hasChanges = emailChanged || usernameChanged || firstNameChanged || lastNameChanged || nickNameChanged || positionChanged || cpaChanged;
                    ...
                    <ReactSelect
                        ...
                        theme={(theme) => ({
                            ...
                            colors: {
                                ...theme.colors,
                                primary: '#00987e',
                                primary25: 'rgba(0, 152, 126, 0.12)',
                                primary50: 'rgba(0, 152, 126, 0.24)',
                                primary75: 'rgba(0, 152, 126, 0.48)',
                            },
                        })}
                        onChange={(opt) => {
                            const nextValue = opt ? (opt as {value: string}).value : '';
                            this.handleCpaValueChange(field.id, nextValue);
                        }}
        ...
        fields.push(
            <label key='position'>
                ...
                <input
                    ...
                    disabled={this.state.isSaving || this.state.isLoading}
                />
                {this.state.fieldErrors.positionField && (
    ...
    handleSubmit = async (event: MouseEvent<HTMLButtonElement>) => {
        ...
        const userFieldsChanged =
            this.state.user.username !== this.state.usernameField ||
            (this.state.user.first_name || '') !== (this.state.firstNameField || '') ||
            (this.state.user.last_name || '') !== (this.state.lastNameField || '') ||
            (this.state.user.position || '') !== (this.state.positionField || '') ||
            (this.state.user.nickname || '') !== (this.state.nickNameField || '');
        ...
        try {
            ...
            // Update user profile if any standard user field changed
            if (emailChanged || userFieldsChanged) {
                const updatedUser = Object.assign({}, this.state.user, {
                    email: this.state.emailField.trim().toLowerCase(),
                    username: this.state.usernameField.trim(),
            ...
            // Handle user update result if user was changed
            if (emailChanged || userFieldsChanged) {
