...
const GroupRow = (props: Props) => {
    ...
    const renderActions = () => {
        if (!props.Workspace_group_id) {
            return null;
        }
        if (props.has_syncables) {
            return (
                <Link
                    to={'/admin_console/user_management/groups/' + props.Workspace_group_id}
                    id={`${props.name}_edit`}
                >
                    <FormattedMessage
    ...
    const renderLinked = () => {
        if (loading) {
            return (
                <a href='#'>
                    <LoadingSpinner
                        text={props.Workspace_group_id ? formatMessage({id: 'admin.group_settings.group_row.unlinking', defaultMessage: 'Unlinking'}) : formatMessage({id: 'admin.group_settings.group_row.linking', defaultMessage: 'Linking'})}
                    />
                </a>
            );
        }
