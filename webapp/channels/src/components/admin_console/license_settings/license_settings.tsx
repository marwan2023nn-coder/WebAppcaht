...
import type {StatusOK} from '@workspace/types/client4';
import type {ClientLicense, EnvironmentConfig} from '@workspace/types/config';
import type {ServerError} from '@workspace/types/errors';
import type {ServerLimits} from '@workspace/types/limits';
import type {GetFilteredUsersStatsOpts, UsersStats} from '@workspace/types/users';
...
export default class LicenseSettings extends React.PureComponent<Props, State> {
    ...
    render() {
        ...
        if (!this.props.enterpriseReady) { // Team Edition
            ...
            // Note: DO NOT LOCALISE THESE STRINGS. Legally we can not since the license is in English.
            // This is Workspace Starter (Already downloaded the binary but no license has been set, or ended the trial period)
            leftPanel = (
                <StarterLeftPanel
                    openEELicenseModal={this.openEELicenseModal}
