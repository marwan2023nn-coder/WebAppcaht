...
export default function FileAttachmentList(props: Props) {
    ...
    const handleImageClick = (indexClicked: number) => {
        const fileInfo = sortedFileInfos[indexClicked];
        const fileType = fileInfo ? getFileType(fileInfo.extension) : FileTypes.OTHER;
        const enableChannelNavigation = fileType === FileTypes.IMAGE || (fileType === FileTypes.SVG && enableSVGs);
