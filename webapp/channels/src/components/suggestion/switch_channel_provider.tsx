...
import {fetchAllMyTeamsChannels, searchAllChannels} from 'workspace-redux/actions/channels';
...
import {getMyPreferences, isGroupChannelManuallyVisible, isCollapsedThreadsEnabled} from 'workspace-redux/selectors/entities/preferences';
import {
    getActiveTeamsList,
    getCurrentTeamId,
    getMyTeams,
    getTeam,
} from 'workspace-redux/selectors/entities/teams';
...
import {sortChannelsByTypeAndDisplayName, isChannelMuted} from 'workspace-redux/utils/channel_utils';
import {getPreferenceKey} from 'workspace-redux/utils/preference_utils';
...
type SwitchChannelProviderOptions = {
...
export default class SwitchChannelProvider extends Provider {
    ...
    private channelsOnly: boolean;
    private currentTeamOnly: boolean;

    constructor(options: SwitchChannelProviderOptions = {}) {
        super();
        this.channelsOnly = options.channelsOnly ?? false;
        this.currentTeamOnly = options.currentTeamOnly ?? false;
    }

    private filterChannelsToCurrentTeam(channels: Channel[]): Channel[] {
        if (!this.currentTeamOnly) {
            return channels;
        }
        ...
        return channels.filter((c) => !c.team_id || c.team_id === teamId);
    }
    ...
    handlePretextChanged(channelPrefix: string, resultsCallback: ResultsCallback<WrappedChannel>) {
        if (channelPrefix) {
            ...
            let channels = getChannelsInAllTeams(state);
            if (!this.channelsOnly) {
                channels = channels.concat(getDirectAndGroupChannels(state));
            }
            channels = channels.filter((c) => c.delete_at === 0);
            channels = this.removeChannelsFromArchivedTeams(channels);
            channels = this.filterChannelsToCurrentTeam(channels);

            if (this.channelsOnly) {
                const channelsOnly = channels.filter((c) => c.type === Constants.OPEN_CHANNEL || c.type === Constants.PRIVATE_CHANNEL);
                const formattedData = this.formatGroup(channelPrefix, channelsOnly, [], true);
                if (formattedData) {
                    resultsCallback(this.initialFilteredList(channelPrefix, formattedData));
                }
            } else {
                const users = searchProfilesMatchingWithTerm(state, channelPrefix, false);
    ...
    async fetchUsersAndChannels(channelPrefix: string, resultsCallback: ResultsCallback<WrappedChannel>) {
        ...
        try {
            ...
            channelsFromServer = ((channelsResponse as ActionResult).data || []) as Channel[];
        ...
        let localChannelData = getChannelsInAllTeams(state);
        if (!this.channelsOnly) {
            localChannelData = localChannelData.concat(getDirectAndGroupChannels(state));
        }
        localChannelData = localChannelData.filter((c) => c.delete_at === 0) || [];
        localChannelData = this.removeChannelsFromArchivedTeams(localChannelData);
        localChannelData = this.filterChannelsToCurrentTeam(localChannelData);
        const localUserData = this.channelsOnly ? [] : searchProfilesMatchingWithTerm(state, channelPrefix, false);
        const localChannelsForFormat = this.channelsOnly ?
            localChannelData.filter((c) => c.type === Constants.OPEN_CHANNEL || c.type === Constants.PRIVATE_CHANNEL) :
            [ThreadsChannel, ...localChannelData];

        const localFormattedData = this.formatGroup(channelPrefix, localChannelsForFormat, localUserData);

        let remoteChannelData: Channel[] = channelsFromServer;
        if (!this.channelsOnly) {
            remoteChannelData = remoteChannelData.concat(getGroupChannels(state)) || [];
        }
        remoteChannelData = this.removeChannelsFromArchivedTeams(remoteChannelData);
        remoteChannelData = this.filterChannelsToCurrentTeam(remoteChannelData);
        if (this.channelsOnly) {
            remoteChannelData = remoteChannelData.filter((c: Channel) => c.type === Constants.OPEN_CHANNEL || c.type === Constants.PRIVATE_CHANNEL);
        }

        const remoteUserData = this.channelsOnly ? [] : (usersFromServer?.users || []);
    ...
    fetchAndFormatRecentlyViewedChannels(resultsCallback: ResultsCallback<WrappedChannel>) {
        const state = this.store.getState();
        let recentChannels = getChannelsInAllTeams(state);
        if (!this.channelsOnly) {
            recentChannels = recentChannels.concat(getDirectAndGroupChannels(state));
        }
        recentChannels = this.removeChannelsFromArchivedTeams(recentChannels);
        recentChannels = this.filterChannelsToCurrentTeam(recentChannels);
        if (this.channelsOnly) {
            recentChannels = recentChannels.filter((c) => c.type === Constants.OPEN_CHANNEL || c.type === Constants.PRIVATE_CHANNEL);
        }
    ...
    wrapChannels(channels: Channel[], channelType: string) {
        ...
        for (let i = 0; i < channels.length; i++) {
            ...
            if (this.channelsOnly && (channel.type === Constants.DM_CHANNEL || channel.type === Constants.GM_CHANNEL)) {
                continue;
            }
    ...
    async fetchChannels(resultsCallback: ResultsCallback<WrappedChannel>) {
        ...
        let filteredChannels = this.filterChannelsToCurrentTeam(channels);
        if (this.channelsOnly) {
            filteredChannels = filteredChannels.filter((c) => c.type === Constants.OPEN_CHANNEL || c.type === Constants.PRIVATE_CHANNEL);
        }
        const sortedChannels = this.wrapChannels(filteredChannels, Constants.MENTION_PUBLIC_CHANNELS).slice(0, 20);
