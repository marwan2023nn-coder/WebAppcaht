...
import {RequestStatus} from 'workspace-redux/constants';
...
jest.mock('workspace-redux/selectors/entities/general', () => ({
    ...jest.requireActual('workspace-redux/selectors/entities/general') as typeof import('workspace-redux/selectors/entities/general'),
    getLicense: () => mockLicense,
    getConfig: () => mockConfig,
}));

jest.mock('workspace-redux/selectors/entities/users', () => ({
    ...jest.requireActual('workspace-redux/selectors/entities/users') as typeof import('workspace-redux/selectors/entities/users'),
    getCurrentUserId: () => '',
}));
...
jest.mock('workspace-redux/actions/users', () => ({
    ...jest.requireActual('workspace-redux/actions/users') as typeof import('workspace-redux/actions/users'),
...
describe('components/signup/Signup', () => {
    ...
    it('should show newsletter check box opt-in for self-hosted non airgapped workspaces', async () => {
        ...
        expect(signupContainer).toHaveTextContent('I would like to receive Workspace security updates via newsletter. By subscribing, I consent to receive emails from Workspace with product updates, promotions, and company news. I have read the Privacy Policy and understand that I can unsubscribe at any time');
    });
    ...
    it('should NOT show newsletter check box opt-in for self-hosted AND airgapped workspaces', async () => {
        ...
        expect(signupContainer).toHaveTextContent('Interested in receiving Workspace security, product, promotions, and company updates updates via newsletter?Sign up at https://workspace.com/security-updates/.');
    });
