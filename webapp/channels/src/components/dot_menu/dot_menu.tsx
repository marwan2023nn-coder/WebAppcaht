...
type Props = {
    ...
    actions: {
        ...
        openDirectChannelToUserId: (userId: string) => Promise<{data?: Channel; error?: any}>;
        switchToChannel: (channel: Channel) => Promise<{data?: any; error?: any}>;
        setFocusPost: (post: Post, targetChannelId?: string) => void;
...
export class DotMenuClass extends React.PureComponent<Props, State> {
    ...
    replyToPostPrivately = async () => {
        const result = await this.props.actions.openDirectChannelToUserId(this.props.post.user_id);

        if (!result.error && result.data) {
            // Mark the focus post BEFORE switching channels so we don't lose it during navigation.
            // We pass the targetChannelId so the ReplyBox opens only after we arrive in the DM channel.
            this.props.actions.setFocusPost(this.props.post, result.data.id);
            await this.props.actions.switchToChannel(result.data);
        }
    };
        ...
        return (
            <Menu.Container
                menuButton={{
                    ...
                    children: this.props.menuButtonChildren || <DotsHorizontalIcon size={16}/>,
                }}
                menu={{
            ...
            >
                ...
                {((this.props.channel?.type === Constants.OPEN_CHANNEL || this.props.channel?.type === Constants.PRIVATE_CHANNEL) && this.props.currentUserId !== this.props.post.user_id && !isSystemMessage) && (
                    <Menu.Item
                        id={`reply_to_post_privately${this.props.post.id}`}
                        data-testid={`reply_to_post_privately${this.props.post.id}`}
                        leadingElement={<ReplyOutlineIcon size={18}/>}
                        labels={
                            <FormattedMessage
                                id='post_info.reply_to_post_privately'
                                defaultMessage='Reply privately'
                            />
                        }
                        onClick={this.replyToPostPrivately}
                    />
                )}
                ...
                {this.state.canEdit && this.props.post.message !== 'BUZZMESSAGE' &&
                    <Menu.Item
