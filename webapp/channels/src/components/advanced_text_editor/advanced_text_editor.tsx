...
import {getChannel, makeGetChannel, getDirectChannel} from 'workspace-redux/selectors/entities/channels';
...
import ReplyBox from 'components/reply_box/reply_box';
...
export type Props = {
    ...
    customSubmit?: (submittingDraft: PostDraft, schedulingInfo?: SchedulingInfo, options?: CreatePostOptions) => Promise<{error?: unknown} | void> | ({error?: unknown} | void);

    showBurnOnRead?: boolean;

    showPostBoxIndicator?: boolean;

    showCallControls?: boolean;

    showPluginControls?: boolean;
}
...
const AdvancedTextEditor = ({
    ...
    customSubmit,
    showBurnOnRead = true,
    showPostBoxIndicator = true,
    showCallControls = true,
    showPluginControls = true,
}: Props) => {
    ...
    const isReplyBoxOpen = useSelector((state: GlobalState) => state.replyBox.isReplyBoxOpen);
    ...
    const prevReplyContextRef = useRef({channelId, rootId});
    ...
    const handleDraftChange = useCallback((draftToChange: PostDraft, options: {instant?: boolean; show?: boolean} = {instant: false, show: false}) => {
        const isCurrentEditorContext = draftToChange.channelId === channelId && draftToChange.rootId === rootId;

        if (isCurrentEditorContext) {
        ...
        const saveDraft = () => {
            ...
            const computedKey = `${prefix}${suffix}`;
            const key = (isCurrentEditorContext && storageKey) ? storageKey : computedKey;
        ...
        if (options.instant || !isCurrentEditorContext) {
            saveDraft();
        } else {
    ...
    }, [channelId, dispatch, rootId, storageKey]);
    ...
    const handleRemoveAllLabels = useCallback(() => {
        ...
        if (updatedDraft.type === PostTypes.BURN_ON_READ) {
            updatedDraft.type = undefined;
        }
    ...
    const sendBuzzMessage = () => {
        const buzz = 'BUZZMESSAGE';
        const updatedDraft = {
            ...draft,
            message: buzz,
            isBuzzMessage: true,
        ...
        // نحتاج مهلة بسيطة حتى تضمن تحديث المسودة في الذاكرة
        setTimeout(() => {
            handleSubmit();
        }, 10);
    };
    ...
    useEffect(() => {
        const prev = prevReplyContextRef.current;
        const replyContextChanged = prev.channelId !== channelId || prev.rootId !== rootId;

        if (replyContextChanged) {
            // Close the ReplyBox when the user navigates between conversations/threads,
            // but do not interfere with reply privately flow that uses focusedPost.
            if (isReplyBoxOpen && !focusedPost) {
                onToggleReplyBox('close');
            }

            prevReplyContextRef.current = {channelId, rootId};
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [channelId, rootId]);
    ...
    useEffect(() => {
        ...
        // Open the reply box when:
        // - Normal reply: focusedPost belongs to the current channel
        // - Reply privately: current channel matches the targetChannelId set by DotMenu
        const shouldOpen = focusTargetChannelId ? (channelId === focusTargetChannelId) : (focusedPost.channel_id === channelId);
        if (shouldOpen) {
            onToggleReplyBox('open', focusedPost);
            dispatch({type: 'CLEAR_FOCUS_POST'});
        }
    }, [channelId, dispatch, focusTargetChannelId, focusedPost, onToggleReplyBox]);
    ...
    ].filter(Boolean), [pluginItems, priorityAdditionalControl, aiRewriteAdditionalControl, isInEditMode, aiRewriteEnabled, burnOnReadAdditionalControl, showBurnOnRead, showPluginControls]);
    ...
    return (
        ...
        >
            ...
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'end'}}>
                {!isInEditMode && showPostBoxIndicator && (
                    <PostBoxIndicator
                        channelId={channelId}
            ...
            >
                ...
                <div
                    ...
                    disabled={isDisabled}
                >
                    {repliedToPost && isReplyBoxOpen && !isInEditMode && (
                        ((location === Locations.CENTER && !repliedToPost.root_id) ||
                        (location === Locations.RHS_COMMENT && repliedToPost.root_id)) &&
                        (
                            <ReplyBox
                                post={repliedToPost}
                    ...
                    >
                        ...
                        <Textbox
                            hasLabels={isInEditMode ? false : Boolean(priorityLabels || (showBurnOnRead && burnOnReadLabels))}
