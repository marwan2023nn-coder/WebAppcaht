...
import {FileTypes} from 'workspace-redux/action_types';
import {getChannelTimezones} from 'workspace-redux/actions/channels';
import {Permissions} from 'workspace-redux/constants';
import {getChannel, getAllChannelStats} from 'workspace-redux/selectors/entities/channels';
import {makeGetFileIdsForPost} from 'workspace-redux/selectors/entities/files';
...
const useSubmit = (
    ...
    const doSubmit = useCallback(async (submittingDraft: PostDraft = draft, schedulingInfo?: SchedulingInfo, createPostOptions?: CreatePostOptions) => {
        ...
        try {
            ...
            if (isInEditMode) {
                ...
                response = await customSubmit(submittingDraft, schedulingInfo, createPostOptions);
            } else {
            ...
            if (isReplyBoxOpen && toggleReplyBox) {
                toggleReplyBox('close');
            }
        } catch (err: unknown) {
    ...
    const handleSubmit = useCallback(async (submittingDraftParam = draft, schedulingInfo?: SchedulingInfo, options?: CreatePostOptions) => {
        ...
        let submittingDraft = setUpdatedFileIds(submittingDraftParam);
        setShowPreview(false);
        isDraftSubmitting.current = true;

        if (isReplyBoxOpen && toggleReplyBox && repliedToPost && !isInEditMode) {
