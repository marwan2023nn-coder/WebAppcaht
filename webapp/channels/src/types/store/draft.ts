...
import type {PostPriority, PostType} from '@workspace/types/posts';
...
export function isPostDraftEmpty(draft: PostDraft): boolean {
    ...
    const hasBurnOnRead = draft.type === PostTypes.BURN_ON_READ || draft.metadata?.burn_on_read?.enabled;
...
export function scheduledPostToPostDraft(scheduledPost: ScheduledPost): PostDraft {
    return {
        ...
        type: (scheduledPost as any).type,
        createAt: 0,
        updateAt: 0,
        metadata: {
