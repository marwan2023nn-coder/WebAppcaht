...
describe('Selectors.Views.ChannelSettings', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(require('workspace-redux/selectors/entities/roles'), 'haveIChannelPermission').mockImplementation(() => false);
        jest.spyOn(require('workspace-redux/selectors/entities/channel_banner'), 'selectChannelBannerEnabled').mockImplementation(() => true);
    });
    ...
    const setPermissionCheckResults = (permissionResults: Record<string, boolean>) => {
        const mockFunction = require('workspace-redux/selectors/entities/roles').haveIChannelPermission as jest.Mock;
