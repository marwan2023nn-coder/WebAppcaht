...
describe('selectors/burn_on_read', () => {
    ...
    describe('getBurnOnReadDurationMinutes', () => {
        ...
        it('should return configured duration from seconds', () => {
            state.entities.general.config.BurnOnReadDurationSeconds = '60';
            const result = getBurnOnReadDurationMinutes(state);
            expect(result).toBe(1);
        });

        it('should return configured duration from legacy minutes key when seconds is missing', () => {
            (state.entities.general.config as any).BurnOnReadDurationMinutes = '15';
            const result = getBurnOnReadDurationMinutes(state);
