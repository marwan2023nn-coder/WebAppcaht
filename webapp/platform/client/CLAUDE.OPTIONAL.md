# CLAUDE: `platform/client/` (`@workspace/client`)

## Purpose
- Implements the Client4 HTTP layer and WebSocket client used by all Workspace web apps and plugins.
...
## Client4 Usage
Singleton HTTP client for all Workspace API requests. Methods return `Promise<ClientResponse<T>>`:
