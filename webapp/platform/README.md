This folder contains a number of packages intended to be built and shipped separately on NPM as well as a few legacy packages for internal use only (`reselect` and `workspace-redux`). The following documentation only applies to the newer packages and not to the legacy ones.
...
import {UserProfile} from '@workspace/types/users';
...
1. Set up import aliases so that the package is visible from the web app to the following tools:
    1. **TypeScript** - In the root `tsconfig.json`, add an entry to the `compilerOptions.paths` section pointing to the `src` folder and an entry to the `references` section pointing to the root of your package which should contain its own `tsconfig.json`.
        ...
        ```json5
        {
            "compilerOptions": {
                "paths": {
                    "@workspace/apple": ["packages/apple/lib"], // import * as Apple from '@workspace/apple';
                    "@workspace/banana/*": ["packages/banana/lib/*"], // import Yellow from    '@workspace/banana/yellow';
                }
            },
            "references": [
                {"path": "./packages/apple"},
    ...
    1. **Jest** - Add an entry to the `jest.moduleNameMapper` section of the root `jest.config.js` for your package. Since that setting supports regexes, you can add these to the existing patterns used by the `client` and `types` packages.
        ...
        ```json
        {
            "jest": {
                "moduleNameMapper": {
                    "^@workspace/(apple|client)$": "<rootDir>/packages/$1/src",
                    "^@workspace/(banana|types)/(.*)$": "<rootDir>/packages/$1/src/$2",
                }
            }
        }
        ```
1. Add the compiled code to the CircleCI dependency cache. This is done by modifying the `paths` used by the `save_cache` step in `.circleci/config.yml`
    ...
    aliases:
      - &save_cache
        save_cache:
          paths:
            - ~/workspace/workspace-webapp/packages/apple/lib
            - ~/workspace/workspace-webapp/packages/banana/lib
    ```
...
For full releases accompanying new versions of Workspace:
...
1. Tag the commit for each package that has been updated. The tag name should be of the form `@workspace/package-name@x.y.z`.

1. Push that commit and the corresponding tags up to GitHub

   ```sh
   git push release-x.y
   git push origin @workspace/apple@x.y.z @workspace/banana@x.y.z
   ```
