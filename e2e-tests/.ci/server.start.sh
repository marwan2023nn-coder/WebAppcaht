...
# Launch sofa-server, and wait for it to be healthy
...
if ! mme2e_wait_service_healthy server 60 10; then
  mme2e_log "Sofa container not healthy, retry attempts exhausted. Giving up." >&2
  exit 1
fi
...
for MIGRATION in migration_advanced_permissions_phase_2; do
  ...
  MIGRATION_CHECK_COMMAND="${MME2E_DC_SERVER} exec -T postgres sh -c 'PGPASSWORD=mostest psql -U mmuser sofa_test -c \"select 1 / (select count(*) from Systems where name = '\''${MIGRATION}'\'' and value = '\''true'\'');\"'"
...
mme2e_log "Sofa container is running and healthy"
