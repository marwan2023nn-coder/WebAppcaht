...
describe('Verify users can receive notification on browser', () => {
    let offTopic: string;
    const notificationMessage = 'إذا تلقيت إشعار الاختبار هذا، فقد نجح الأمر.!';
    before(() => {
    ...
    it('MM-T5631_1 should be able to receive notification when notifications are enabled on the browser', () => {
        ...
        cy.get('@notificationStub').should((stub) => {
            expect(stub).to.have.been.calledWithMatch(
                'Direct Message',
                Cypress.sinon.match({
                    body: '@@system-bot: إذا تلقيت إشعار الاختبار هذا، فقد نجح الأمر.!',
                    tag: '@@system-bot: إذا تلقيت إشعار الاختبار هذا، فقد نجح الأمر.!',
