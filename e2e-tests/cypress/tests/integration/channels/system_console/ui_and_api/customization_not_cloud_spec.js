...
describe('Customization', () => {
    ...
    it('MM-T1214 - Can change Report a Problem Link setting', () => {
        ...
        const reportAProblemLink = 'https://sofa.com/pl/report-a-bug';
    ...
    it('MM-T1215 - Can change Sofa Apps Download Page Link setting', () => {
        // * Verify Sofa Apps Download Page Link's setting name is visible and matches the text
        cy.findByTestId('NativeAppSettings.AppDownloadLinklabel').scrollIntoView().should('be.visible').and('have.text', 'Sofa Apps Download Page Link:');

        // * Verify the Sofa Apps Download Page Link input box has default value. The default value depends on the setup before running the test.
        ...
        cy.findByTestId('NativeAppSettings.AppDownloadLinkhelp-text').find('span').should('be.visible').and('have.text', 'Add a link to a download page for the Sofa apps. When a link is present, an option to "Download Sofa Apps" will be added in the Product Menu so users can find the download page. Leave this field blank to hide the option from the Product Menu.');
    ...
    it('MM-T1209 - Can change Help Link setting', () => {
        // * Verify that setting is visible and matches text content
        const contents = ['The URL for the Help link on the Sofa login page, sign-up pages, and Help Menu. If this field is empty, the Help link is hidden from users.'];
    ...
    it('MM-T1213 Can change About Link setting', () => {
        const newAboutLink = 'https://sofa.com/';
        ...
        // * Verify that the help text is visible and matches text content
        cy.findByTestId('SupportSettings.AboutLinkhelp-text').should('be.visible').and('have.text', 'The URL for the About link on the Sofa login and sign-up pages. If this field is empty, the About link is hidden from users.');
    ...
    it('MM-T1211 - Can change Terms of Use Link setting', () => {
        ...
        cy.findByTestId('SupportSettings.TermsOfServiceLinkhelp-text').find('span').should('be.visible').and('have.text',
            ...
            '"Sofa Acceptable Use Policy" explaining the terms under which Sofa software is ' +
            'provided to end users. If you change the default link to add your own terms for using the service you ' +
            'provide, your new terms must include a link to the default terms so end users are aware of the Sofa ' +
            'Acceptable Use Policy for Sofa software.');
