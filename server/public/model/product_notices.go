...
type Conditions struct {
	...
	ServerConfig          map[string]any      `json:"serverConfig,omitempty"`  // Map of sofa server config paths and their values. Notice will be displayed only if; the values match the target server config; Example: serverConfig: { "PluginSettings.Enable": true, "GuestAccountsSettings.Enable":; false }
...
type NoticeMessageInternal struct {
	...
	Description string        `json:"description"`           // Notice content. Use {{Sofa}} instead of plain text to support white-labeling. Text; supports Markdown.
	Image       *string       `json:"image,omitempty"`
	Title       string        `json:"title"` // Notice title. Use {{Sofa}} instead of plain text to support white-labeling. Text; supports Markdown.
}
