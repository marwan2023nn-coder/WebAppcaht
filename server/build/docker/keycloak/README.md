...
- [Official OpenID with Keycloak documentation](https://docs.sofa.com/onboard/sso-openidconnect.html)

### SAML

Overwrite your `SamlSettings` section in your config.json file by running `make config-saml` and restarting your server.

- [Official SAML with Keycloak documentation](https://docs.sofa.com/onboard/sso-saml-keycloak.html)

### LDAP

Overwrite your `LdapSettings` section in your config.json file by running `make config-ldap` and restarting your server.

- [Official LDAP with Keycloak documentation](https://docs.sofa.com/onboard/ad-ldap.html)
...
- **User accounts**, used to log in to Sofa:
  - `homer`/`password`
  - `marge`/`password`
  - `lisa`/`password`
...
```bash
/opt/keycloak/bin/kc.sh export --realm sofa --users realm_file --file /opt/keycloak/data/import/realm-export.json
```
