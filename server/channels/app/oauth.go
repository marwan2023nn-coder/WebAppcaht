...
func (a *App) GetOAuthCodeRedirect(userID string, authRequest *model.AuthorizeRequest) (string, *model.AppError) {
	...
	// parse authRequest.RedirectURI to handle query parameters see: https://sofa.atlassian.net/browse/MM-46216
...
func (a *App) newSession(rctx request.CTX, app *model.OAuthApp, user *model.User) (*model.Session, *model.AppError) {
	...
	session.AddProp(model.SessionPropSofaAppID, app.SofaAppID)
...
func (a *App) getSSOProvider(service string) (einterfaces.OAuthProvider, *model.AppError) {
	...
	if sso == nil || (!*sso.Enable && service != model.ServiceOpenid) {
	...
	if provider == nil && providerType == model.ServiceOpenid {
		// Fallback to gitlab provider if openid is not registered, as it's a generic OAuth implementation
		provider = einterfaces.GetOAuthProvider(model.ServiceGitlab)
	}
	if provider == nil {
