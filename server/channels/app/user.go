...
func (a *App) GetViewUsersRestrictions(rctx request.CTX, userID string) (*model.ViewUsersRestrictions, *model.AppError) {
	...
	// Instead of calling HasPermissionToTeam in a loop O(N teams), use a single
	// cached SQL query to get all teams where the user is SchemeAdmin.
	// This eliminates N sequential DB round-trips for users in many teams.
	teamIDsWithPermission, nErr2 := a.Srv().Store().Team().GetAdminTeamIdsForUser(userID, true)
	if nErr2 != nil {
		return nil, model.NewAppError("GetViewUsersRestrictions", "app.team.get_admin_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(nErr2)
	}
...
func (a *App) DeactivateInactiveUsers(rctx request.CTX, days int) (int64, *model.AppError) {
	threshold := model.GetMillis() - int64(days*24*60*60*1000)
	count, err := a.Srv().Store().User().DeactivateInactiveUsers(threshold)
	if err != nil {
		return 0, model.NewAppError("DeactivateInactiveUsers", "app.user.update_active_for_multiple_users.updating.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
	}

	if count > 0 {
		// FIXED (Issue #4 — Nuclear Cache Invalidation):
		// The previous `InvalidateAllCaches()` flushed EVERY server cache (posts, channels,
		// roles, sessions, emojis, etc.), causing a "thundering herd" of DB queries on the
		// next request from any user, leading to spikes on low-resource servers.
		//
		// Scoped fix:
		//   1. Clear only the user-related cache (the data that actually changed).
		//   2. Broadcast a targeted WebSocket event so connected clients re-evaluate
		//      their own session status gracefully, without a full server cache flush.
		a.Srv().Store().User().ClearCaches()

		// Broadcast a user_updated event so clients re-check authentication.
		// Using an empty userId broadcasts to all connections — each client will then
		// verify its own session validity on the next action, avoiding a single spike.
		message := model.NewWebSocketEvent(model.WebsocketEventUserUpdated, "", "", "", nil, "")
		a.Publish(message)

		rctx.Logger().Info("DeactivateInactiveUsers: scoped cache invalidation complete",
			mlog.Int("deactivated_count", int(count)),
		)
	}
