...
func (a *App) CreatePost(rctx request.CTX, post *model.Post, channel *model.Channel, flags model.CreatePostFlags) (savedPost *model.Post, isMemberForPreviews bool, err *model.AppError) {
	...
	// Block post creation when the license has been removed or expired.
	// System message posts (e.g. from bots/plugins/Calls) are always allowed through.
	if !strings.HasPrefix(post.Type, model.PostSystemMessagePrefix) {
		if !a.Srv().Platform().IsLicenseActive() {
			return nil, false, model.NewAppError("CreatePost", "api.license.expired.app_error", nil, "no active license", http.StatusForbidden)
		}
	}
