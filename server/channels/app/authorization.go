...
func (a *App) HasProfileEditPermission(rctx request.CTX, userId string) bool {
	// System Admin can always edit any profile
	...
	for _, role := range roles {
		if role == model.SystemUserManagerRoleId {
			hasSystemUserManager = true
			break
		}
	}

	if !hasSystemUserManager {
		return false
	}

	// If they have system_user_manager, check if they manage at least one team the target user is in
	sessionUserId := rctx.Session().UserId

	adminTeamIds, err := a.Srv().Store().Team().GetAdminTeamIdsForUser(sessionUserId, true)
	if err != nil {
		return false
	}
	...
	targetUserTeamIds, err := a.Srv().Store().Team().GetUserTeamIds(userId, true)
	if err != nil {
		return false
	}

	for _, adminId := range adminTeamIds {
		for _, targetId := range targetUserTeamIds {
			if adminId == targetId {
				return true
			}
		}
