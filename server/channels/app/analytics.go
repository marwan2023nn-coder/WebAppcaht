...
func (a *App) getExtraCountsAnalytics(rctx request.CTX, teamID string) (model.AnalyticsRows, *model.AppError) {
	...
	rows[6] = &model.AnalyticsRow{Name: "total_attached_file_count", Value: 0}
	rows[7] = &model.AnalyticsRow{Name: "total_downloaded_file_count", Value: 0}
	...
	var attachedFileCount int64
	g.Go(func() error {
		var err error
		if attachedFileCount, err = a.Srv().Store().FileInfo().CountAttached(); err != nil {
			return model.NewAppError("GetAnalytics", "app.file_info.get_attached_count.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
		}
		return nil
	})

	var downloadedFileCount int64
	g.Go(func() error {
		var err error
		if downloadedFileCount, err = a.Srv().Store().FileInfo().CountDownloaded(); err != nil {
			return model.NewAppError("GetAnalytics", "app.file_info.get_downloaded_count.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
		}
		return nil
	})
	...
	rows[6].Value = float64(attachedFileCount)
	rows[7].Value = float64(downloadedFileCount)
