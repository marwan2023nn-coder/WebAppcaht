...
func NewChannels(s *Server) (*Channels, error) {
	...
	if complianceInterface != nil {
		...
		ch.Compliance = &complianceStub{}
	}
	if messageExportInterface != nil {
		ch.MessageExport = messageExportInterface(New(ServerConnector(ch)))
	} else {
		ch.MessageExport = &messageExportStub{}
	}
	if dataRetentionInterface != nil {
		ch.DataRetention = dataRetentionInterface(New(ServerConnector(ch)))
	} else {
		// Use a no-op stub so Data Retention API endpoints return empty-but-valid
		// responses instead of 501 "license does not support" errors.
		ch.DataRetention = &dataRetentionStub{}
	}
