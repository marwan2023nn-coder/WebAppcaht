...
func TestSendFlaggedPostRemovalNotification(t *testing.T) {
	...
	t.Run("should send notifications to all configured targets", func(t *testing.T) {
		...
		reviewerMessage := fmt.Sprintf("تم حذف الرسالة المبلغ عنها بواسطة @%s\n\nبإضافة تعليق:\n\n> %s", th.SystemAdminUser.Username, actorComment)
		...
		authorMessage := fmt.Sprintf("تم حذف منشورك صاحب المعرف `%s` في القناة `%s` الذي تم الإبلاغ عنه للمراجعة نهائيًا بواسطة المراجع.", post.Id, th.BasicChannel.DisplayName)
		...
		reporterMessage := fmt.Sprintf("تم حذف المنشور الذي يحمل المعرف `%s` في القناة `%s` والذي قمت بالإبلاغ عنه للمراجعة نهائيًا بواسطة المراجع.", post.Id, th.BasicChannel.DisplayName)
	...
	t.Run("should send notifications only to configured targets", func(t *testing.T) {
		...
		expectedMessage := fmt.Sprintf("تم حذف الرسالة المبلغ عنها بواسطة @%s\n\nبإضافة تعليق:\n\n> %s", th.SystemAdminUser.Username, "Test comment")
	...
	t.Run("should handle empty comment", func(t *testing.T) {
		...
		expectedMessage := fmt.Sprintf("تم حذف الرسالة المبلغ عنها بواسطة @%s", th.SystemAdminUser.Username)
...
func TestSendKeepFlaggedPostNotification(t *testing.T) {
	...
	t.Run("should send notifications to all configured targets", func(t *testing.T) {
		...
		// Verify reviewer message
		reviewerMessage := fmt.Sprintf("تم الإبقاء على الرسالة المبلغ عنها بواسطة @%s\n\nبإضافة تعليق:\n\n> %s", th.SystemAdminUser.Username, actorComment)
		var reviewerPost *model.Post
		for _, p := range createdPosts {
			if p.Message == reviewerMessage {
				reviewerPost = p
		...
		// Verify author message
		authorMessage := fmt.Sprintf("تمت استعادة منشورك صاحب المعرف `%s` في القناة `%s` الذي تم الإبلاغ عنه للمراجعة بواسطة المراجع.", post.Id, th.BasicChannel.DisplayName)
		var authorPost *model.Post
		for _, p := range createdPosts {
			if p.Message == authorMessage {
				authorPost = p
		...
		// Verify reporter message
		reporterMessage := fmt.Sprintf("تمت استعادة المنشور صاحب المعرف `%s` في القناة `%s` الذي قمت بالإبلاغ عنه للمراجعة بواسطة المراجع.", post.Id, th.BasicChannel.DisplayName)
		var reporterPost *model.Post
		for _, p := range createdPosts {
			if p.Message == reporterMessage {
				reporterPost = p
	...
	t.Run("should send notifications only to configured targets", func(t *testing.T) {
		...
		expectedMessage := fmt.Sprintf("تم الإبقاء على الرسالة المبلغ عنها بواسطة @%s\n\nبإضافة تعليق:\n\n> %s", th.SystemAdminUser.Username, comment)
		verifyNotificationPost(t, createdPosts[0], expectedMessage, contentReviewBot.UserId, createdPosts[0].ChannelId)
	})
	...
	t.Run("should handle empty comment", func(t *testing.T) {
		...
		expectedMessage := fmt.Sprintf("تم الإبقاء على الرسالة المبلغ عنها بواسطة @%s", th.SystemAdminUser.Username)
		verifyNotificationPost(t, createdPosts[0], expectedMessage, createdPosts[0].UserId, createdPosts[0].ChannelId)
	})
