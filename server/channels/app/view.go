...
func (a *App) GetPostsForView(rctx request.CTX, options model.GetPostsOptions) (*model.PostList, *model.AppError) {
	// For now, views return all posts in the channel. In the future, this will
	// filter based on the view configuration (e.g. board property values).
	return a.GetPosts(options)
}
