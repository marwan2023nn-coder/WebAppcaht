...
func (ts *TeamService) JoinUserToTeam(rctx request.CTX, team *model.Team, user *model.User) (*model.TeamMember, bool, error) {
	...
	// Note: Removed automatic Team Admin assignment based on email match (team.Email == user.Email)
	// New users should be added as regular members unless explicitly promoted through other mechanisms
