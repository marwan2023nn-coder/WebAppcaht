...
func TestHandlerServeCSPHeader(t *testing.T) {
	...
	t.Run("static, without subpath", func(t *testing.T) {
		...
		handler.ServeHTTP(response, request)
		assert.Equal(t, 200, response.Code)
		assert.Equal(t, []string{"frame-ancestors 'self' " + *th.App.Config().ServiceSettings.FrameAncestors + "; script-src 'self' js.stripe.com/v3; connect-src 'self' ws: wss: https: blob: data: chrome-extension: moz-extension: safari-extension: edge-extension: extension:; worker-src 'self' blob: chrome-extension: moz-extension: safari-extension: edge-extension: extension:"}, response.Header()["Content-Security-Policy"])
		assert.Equal(t, "0", response.Header().Get("X-XSS-Protection"))
	})
	...
	t.Run("dev mode", func(t *testing.T) {
		...
		handler.ServeHTTP(response, request)
		assert.Equal(t, 200, response.Code)
		assert.Equal(t, []string{"frame-ancestors 'self' " + *th.App.Config().ServiceSettings.FrameAncestors + "; script-src 'self' js.stripe.com/v3 'unsafe-eval' 'unsafe-inline'; connect-src 'self' ws: wss: https: blob: data: chrome-extension: moz-extension: safari-extension: edge-extension: extension:; worker-src 'self' blob: chrome-extension: moz-extension: safari-extension: edge-extension: extension:"}, response.Header()["Content-Security-Policy"])
	})
}
