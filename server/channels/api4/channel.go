...
func getAllChannels(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// For non-system admins, filter to only channels in their managed teams
	// Also exclude Group and Direct Message channels (they don't belong to teams)
	if !c.IsSystemAdmin() && len(channels) > 0 {
		userTeamIDs, err := c.App.Srv().Store().Team().GetAdminTeamIdsForUser(c.AppContext.Session().UserId, true)
		if err != nil {
			c.Err = model.NewAppError("getAllChannels", "app.team.get_user_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}
		...
		filteredChannels := make(model.ChannelListWithTeamData, 0)
		for _, channel := range channels {
			// Skip Group and Direct Message channels
			if channel.Type == model.ChannelTypeGroup || channel.Type == model.ChannelTypeDirect {
				continue
			}
			// Only include channels from managed teams
			if userTeamIDsMap[channel.TeamId] {
...
func searchAllChannels(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.IsSystemAdmin() && len(channels) > 0 {
		userTeamIDs, err := c.App.Srv().Store().Team().GetAdminTeamIdsForUser(c.AppContext.Session().UserId, true)
		if err != nil {
			c.Err = model.NewAppError("searchAllChannels", "app.team.get_user_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}
		...
		channels = filteredChannels
		totalCount = int64(len(filteredChannels))
	}
