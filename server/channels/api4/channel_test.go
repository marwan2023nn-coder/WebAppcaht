...
func TestGetChannelsForUser(t *testing.T) {
	...
	for _, ch := range channels {
		...
		if ch.DisplayName == "المجموعة الفرعية" {
			numOffTopic++
		} else if ch.DisplayName == "المجموعة الرئيسية" {
			numTownSquare++
		}
	}
...
func TestGetAllChannels(t *testing.T) {
	...
	t.Run("verify correct sanitization", func(t *testing.T) {
		...
		for _, channel := range channels {
			if channel.DisplayName != "المجموعة الفرعية" && channel.DisplayName != "المجموعة الرئيسية" {
