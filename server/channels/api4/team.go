...
func getAllTeams(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// For non-system admins (e.g. Community Managers), filter to only their admin teams
	if !c.IsSystemAdmin() {
		var err error
		opts.TeamIds, err = c.App.Srv().Store().Team().GetAdminTeamIdsForUser(c.AppContext.Session().UserId, true)
		if err != nil {
			c.Err = model.NewAppError("getAllTeams", "app.team.get_user_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}
	}
...
func searchTeams(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// For non-system admins, filter to only their teams
	if !c.IsSystemAdmin() && len(teams) > 0 {
		userTeamIDs, err := c.App.Srv().Store().Team().GetAdminTeamIdsForUser(c.AppContext.Session().UserId, true)
		if err != nil {
			c.Err = model.NewAppError("searchTeams", "app.team.get_user_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}

		userTeamIDsMap := make(map[string]bool)
		for _, id := range userTeamIDs {
			userTeamIDsMap[id] = true
		}

		filteredTeams := make([]*model.Team, 0)
		for _, team := range teams {
			if userTeamIDsMap[team.Id] {
...
func inviteGuestsToChannels(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	guestEnabled := license != nil && license.Features != nil && model.SafeDereference(license.Features.GuestAccounts)
