...
func getUsersForReporting(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// For non-system admins (e.g. Community Managers), we enforce strict scoping
	// regardless of whether they have global view permissions (PermissionViewMembers).
	// This ensures Admin Console lists are always scoped to their managed teams.
	if !c.IsSystemAdmin() {
		// Get teams where user is Team Admin (SchemeAdmin = true)
		teamIDs, err := c.App.Srv().Store().Team().GetAdminTeamIdsForUser(c.AppContext.Session().UserId, true)
		if err != nil {
			c.Err = model.NewAppError("getUsersForReporting", "app.team.get_user_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}

		// If they manage no teams, they see no one (empty restrictions list = match nothing)
		// ViewUsersRestrictions with empty lists implies "no access" in applyViewRestrictionsFilter
		// unless we explicitly handle nil differently.
		// applyViewRestrictionsFilter: "If you have no access to teams or channels, return and empty result."
		options.ViewRestrictions = &model.ViewUsersRestrictions{Teams: teamIDs, Channels: []string{}}
	} else {
		// For System Admins, or if we want to fallback to standard logic (though usually SysAdmins see all)
		restriction, err := c.App.GetViewUsersRestrictions(c.AppContext, c.AppContext.Session().UserId)
		if err != nil {
			c.Err = err
			return
		}
		options.ViewRestrictions = restriction
	}
...
func getUserCountForReporting(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.IsSystemAdmin() {
		teamIDs, err := c.App.Srv().Store().Team().GetAdminTeamIdsForUser(c.AppContext.Session().UserId, true)
		if err != nil {
			c.Err = model.NewAppError("getUserCountForReporting", "app.team.get_user_team_ids.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			return
		}
		options.ViewRestrictions = &model.ViewUsersRestrictions{Teams: teamIDs, Channels: []string{}}
	} else {
