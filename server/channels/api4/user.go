...
func (api *API) InitUser() {
	...
	api.BaseRoutes.Users.Handle("/login", api.RateLimitedHandler(api.RateLimitedHandler(api.APIHandler(login), model.RateLimitSettings{PerSec: model.NewPointer(5), MaxBurst: model.NewPointer(10)}), model.RateLimitSettings{PerSec: model.NewPointer(5), MaxBurst: model.NewPointer(10)})).Methods(http.MethodPost)
	...
	api.BaseRoutes.Users.Handle("/deactivate/inactive", api.APISessionRequired(deactivateInactiveUsers)).Methods(http.MethodPost)
...
// isTeamAdminInAnyTeam checks if the user is a Team Admin (SchemeAdmin) in at least one team.
func isTeamAdminInAnyTeam(session *model.Session) bool {
	if session == nil {
		return false
	}
	for _, teamMember := range session.TeamMembers {
		if teamMember != nil && teamMember.SchemeAdmin {
			return true
		}
	}
	return false
...
func createUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	} else if c.IsSystemAdmin() || (c.App.SessionHasPermissionTo(*c.AppContext.Session(), model.PermissionSysconsoleWriteUserManagementUsers) && isTeamAdminInAnyTeam(c.AppContext.Session())) {
		ruser, err = c.App.CreateUserAsAdmin(c.AppContext, &user, redirect)
		auditRec.AddMeta("admin", true)

		// If the creator is a User Manager (not System Admin), automatically add the new user
		// to teams where the manager is a Team Admin (SchemeAdmin).
		if err == nil && !c.IsSystemAdmin() {
			// Get the creator's team memberships from the session
			for _, teamMember := range c.AppContext.Session().TeamMembers {
				// Only add to teams where the creator is a Team Admin
				if teamMember != nil && teamMember.SchemeAdmin {
					if teamJoinErr := c.App.AddUserToTeamByTeamId(c.AppContext, teamMember.TeamId, ruser); teamJoinErr != nil {
						c.Logger.Warn("Failed to automatically add new user to team", mlog.String("team_id", teamMember.TeamId), mlog.String("user_id", ruser.Id), mlog.Err(teamJoinErr))
					}
				}
			}
		}
...
func setProfileImage(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.HasProfileEditPermission(c.AppContext, c.Params.UserId) {
...
func updateUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.HasProfileEditPermission(c.AppContext, user.Id) {
		c.SetPermissionError(model.PermissionEditOtherUsers)
		return
	}
...
func getLoginType(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !*c.App.Config().GuestAccountsSettings.EnableGuestMagicLink ||
		...
		license == nil || license.Features == nil || !model.SafeDereference(license.Features.GuestAccounts) {
...
type deactivateInactiveUsersRequest struct {
	Days int `json:"days"`
}

func deactivateInactiveUsers(c *Context, w http.ResponseWriter, r *http.Request) {
	if !c.App.SessionHasPermissionTo(*c.AppContext.Session(), model.PermissionSysconsoleWriteUserManagementUsers) {
		c.SetPermissionError(model.PermissionSysconsoleWriteUserManagementUsers)
		return
	}

	var req deactivateInactiveUsersRequest
	...
	auditRec := c.MakeAuditRecord(model.AuditEventUpdateUserActive, model.AuditStatusFail)
	defer c.LogAuditRec(auditRec)
	model.AddEventParameterToAuditRec(auditRec, "days", req.Days)

	count, err := c.App.DeactivateInactiveUsers(c.AppContext, req.Days)
	if err != nil {
		c.Err = err
		return
	}

	model.AddEventParameterToAuditRec(auditRec, "count", count)
	auditRec.Success()
