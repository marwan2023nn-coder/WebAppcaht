...
func uploadFileStream(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// --- Sofa Workspace: License enforcement ---
	// Block file uploads when the license was removed or expired.
	if !c.App.Srv().Platform().IsLicenseActive() {
		c.Err = model.NewAppError("uploadFileStream", "api.license.expired.app_error", nil, "no active license", http.StatusForbidden)
		return
	}
...
func getFile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// Track download count (non-blocking, ignore errors)
	go func() {
		_ = c.App.Srv().Store().FileInfo().IncrementDownloadCount(fileInfo.Id)
	}()
