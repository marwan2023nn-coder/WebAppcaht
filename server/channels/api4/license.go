...
func getClientLicense(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// FIXED (Issue #2 — Race Condition):
	// ClientLicense() returns a reference to the shared map stored inside an atomic.Pointer.
	// Mutating it directly would cause concurrent-write data races across goroutines.
	// We create a shallow copy (per-request, stack-friendly allocation) before adding keys.
	original := c.App.Srv().Platform().ClientLicense()
	clientLicense := make(map[string]string, len(original)+5)
	...
	if clientLicense["IsLicensed"] != "false" && clientLicense["IsLicensed"] != "" {
		...
		clientLicense["EnterprisePlugins"] = "true"
	}
...
func addLicense(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// FIXED (Issue #6 — Double RSA Validation):
	// The previous code called `utils.LicenseValidator.LicenseFromBytes(licenseBytes)` here,
	// which performs a full RSA signature verification — an expensive CPU operation.
	// This was done solely to call `license.IsSanctionedTrial()` before `SaveLicense`.
	// However, `SaveLicense` calls `LicenseFromBytes` internally as well, resulting in
	// TWO RSA verifications for every license upload.
	//
	// Solution: Use `licenseFromBytesNoSignatureCheck` to do a lightweight JSON-only decode
	// (no RSA) just to extract the trial flags. The single authoritative RSA validation
	// happens exactly once inside `SaveLicense` below.
	//
	// CPU cost: RSA 2048-bit verify ≈ 0.5-1ms on low-resource servers.
	// Eliminated: 1 redundant RSA operation per license upload.
	preCheckLicense := licenseFromBytesNoSignatureCheck(licenseBytes)

	// skip the restrictions if license is a sanctioned trial
	if preCheckLicense != nil && !preCheckLicense.IsSanctionedTrial() && preCheckLicense.IsTrialLicense() {
	...
	// SaveLicense performs the single authoritative RSA validation here.
...
// licenseFromBytesNoSignatureCheck performs a lightweight JSON-only decode of a license file
// without any RSA signature verification. Used solely for pre-checking trial flags before
// the authoritative validation in SaveLicense.
//
// The base64-encoded license format is: base64(JSON_payload + RSA_signature_256_bytes).
// We decode and strip the trailing 256 bytes (RSA-2048 signature) to get the JSON.
// This is intentionally NOT a security check — it's a pre-screen for trial license logic.
func licenseFromBytesNoSignatureCheck(licenseBytes []byte) *model.License {
	decoded := make([]byte, base64.StdEncoding.DecodedLen(len(licenseBytes)))
	n, err := base64.StdEncoding.Decode(decoded, licenseBytes)
	if err != nil || n <= 256 {
		return nil
	}
	jsonBytes := decoded[:n-256]
	var lic model.License
	if jsonErr := json.Unmarshal(jsonBytes, &lic); jsonErr != nil {
		return nil
	}
	return &lic
}
...
func getPrevTrialLicense(c *Context, w http.ResponseWriter, r *http.Request) {
	// Bypass LicenseManager check and return empty JSON to simulate no previous trial
