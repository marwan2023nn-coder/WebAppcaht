...
func TestUserStore(t *testing.T, rctx request.CTX, ss store.Store, s SqlStore) {
	...
	t.Run("DeactivateInactiveUsers", func(t *testing.T) { testUserStoreDeactivateInactiveUsers(t, rctx, ss, s) })
}
...
func testUserStoreDeactivateInactiveUsers(t *testing.T, rctx request.CTX, ss store.Store, s SqlStore) {
	// Create a regular user
	u1 := &model.User{Email: MakeEmail(), Username: model.NewUsername()}
	u1, err := ss.User().Save(rctx, u1)
	require.NoError(t, err)
	defer ss.User().PermanentDelete(rctx, u1.Id)

	// Create an admin user (should not be deactivated)
	admin := &model.User{Email: MakeEmail(), Username: model.NewUsername(), Roles: model.SystemAdminRoleId}
	admin, err = ss.User().Save(rctx, admin)
	require.NoError(t, err)
	defer ss.User().PermanentDelete(rctx, admin.Id)

	// Create a bot user (should not be deactivated)
	botUser := &model.User{Email: MakeEmail(), Username: model.NewUsername()}
	botUser, err = ss.User().Save(rctx, botUser)
	require.NoError(t, err)
	defer ss.User().PermanentDelete(rctx, botUser.Id)
	_, nErr := ss.Bot().Save(&model.Bot{UserId: botUser.Id, Username: botUser.Username, OwnerId: u1.Id})
	require.NoError(t, nErr)
	defer ss.Bot().PermanentDelete(botUser.Id)

	// Set last activity for u1 to be old (e.g., 40 days ago)
	thresholdDays := int64(30)
	oldActivity := model.GetMillis() - (thresholdDays+10)*DayMilliseconds

	err = ss.Status().SaveOrUpdate(&model.Status{UserId: u1.Id, Status: model.StatusOffline, Manual: false, LastActivityAt: oldActivity})
	require.NoError(t, err)

	// Set last activity for admin to be old (should be ignored)
	err = ss.Status().SaveOrUpdate(&model.Status{UserId: admin.Id, Status: model.StatusOffline, Manual: false, LastActivityAt: oldActivity})
	require.NoError(t, err)

	// Set last activity for bot to be old (should be ignored)
	err = ss.Status().SaveOrUpdate(&model.Status{UserId: botUser.Id, Status: model.StatusOffline, Manual: false, LastActivityAt: oldActivity})
	require.NoError(t, err)

	// Run deactivation using the threshold timestamp directly
	thresholdTime := model.GetMillis() - thresholdDays*DayMilliseconds
	count, err := ss.User().DeactivateInactiveUsers(thresholdTime)
	require.NoError(t, err)
	require.Equal(t, int64(1), count)

	// Verify u1 is deactivated
	u1_updated, err := ss.User().Get(context.Background(), u1.Id)
	require.NoError(t, err)
	require.NotEqual(t, int64(0), u1_updated.DeleteAt)

	// Verify admin is NOT deactivated
	admin_updated, err := ss.User().Get(context.Background(), admin.Id)
	require.NoError(t, err)
	require.Equal(t, int64(0), admin_updated.DeleteAt)

	// Verify bot is NOT deactivated
	bot_updated, err := ss.User().Get(context.Background(), botUser.Id)
	require.NoError(t, err)
	require.Equal(t, int64(0), bot_updated.DeleteAt)
}
