...
func testGetChannelUnreadsForAllTeams(t *testing.T, rctx request.CTX, ss store.Store) {
	...
	c1 := &model.Channel{TeamId: m1.TeamId, Name: model.NewId(), DisplayName: "المجموعة الرئيسية", Type: model.ChannelTypeOpen, TotalMsgCount: 100}
	_, nErr = ss.Channel().Save(rctx, c1, -1)
	require.NoError(t, nErr)

	c2 := &model.Channel{TeamId: m2.TeamId, Name: model.NewId(), DisplayName: "المجموعة الرئيسية", Type: model.ChannelTypeOpen, TotalMsgCount: 100}
...
func testGetChannelUnreadsForTeam(t *testing.T, rctx request.CTX, ss store.Store) {
	...
	c2 := &model.Channel{TeamId: m1.TeamId, Name: model.NewId(), DisplayName: "المجموعة الرئيسية", Type: model.ChannelTypeOpen, TotalMsgCount: 100}
	_, nErr = ss.Channel().Save(rctx, c2, -1)
	require.NoError(t, nErr)
