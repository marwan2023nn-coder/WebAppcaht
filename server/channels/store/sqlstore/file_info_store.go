...
func (fs SqlFileInfoStore) CountAttached() (int64, error) {
	query := fs.getQueryBuilder().
		Select("COUNT(Id)").
		From("FileInfo").
		Where(sq.NotEq{"PostId": ""}).
		Where(sq.Eq{"DeleteAt": 0})

	var count int64
	err := fs.GetReplica().GetBuilder(&count, query)
	if err != nil {
		return int64(0), errors.Wrap(err, "failed to count attached Files")
	}
	return count, nil
}

func (fs SqlFileInfoStore) CountDownloaded() (int64, error) {
	query := fs.getQueryBuilder().
		Select("COUNT(Id)").
		From("FileInfo").
		Where(sq.Gt{"DownloadCount": 0}).
		Where(sq.Eq{"DeleteAt": 0})

	var count int64
	err := fs.GetReplica().GetBuilder(&count, query)
	if err != nil {
		return int64(0), errors.Wrap(err, "failed to count downloaded Files")
	}
	return count, nil
}

func (fs SqlFileInfoStore) IncrementDownloadCount(fileId string) error {
	query := fs.getQueryBuilder().
		Update("FileInfo").
		Set("DownloadCount", sq.Expr("DownloadCount + 1")).
		Where(sq.Eq{"Id": fileId})

	_, err := fs.GetMaster().ExecBuilder(query)
	if err != nil {
		return errors.Wrapf(err, "failed to increment download count for file %s", fileId)
	}
	return nil
}
