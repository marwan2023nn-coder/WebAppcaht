...
func getUsersColumnsWithName(name string, extraColumns ...string) []string {
	...
	for i, col := range columns {
		if col == "Email" {
			result[i] = fmt.Sprintf("COALESCE(%s.Email, '') AS Email", name)
		} else {
			result[i] = name + "." + col
		}
	}
...
func (us SqlUserStore) insert(user *model.User) (sql.Result, error) {
	...
	query := `INSERT INTO Users
		...
		VALUES
		(:id, :create_at, :update_at, :delete_at, :username, :password, :auth_data, :auth_service,
			:email, :email_verified, :nickname, :first_name, :last_name, :position, :roles, :allow_marketing,
			:props, :notify_props, :last_password_update, :last_picture_update, :failed_attempts,
			:locale, :timezone, :mfa_active, :mfa_secret, :remote_id, :mfa_used_timestamps)`

	user.Props = wrapBinaryParamStringMap(us.IsBinaryParamEnabled(), user.Props)

	// If email is empty, we must ensure it's saved as NULL in the database
	// to avoid unique constraint violations since multiple empty strings are not allowed
	// but multiple NULLs are (in Postgres/MySQL).
	// We use a temporary map for NamedExec to handle the NULL email.
	userMap := map[string]any{
		...
		"roles":                user.Roles,
		"allow_marketing":      user.AllowMarketing,
		"props":                user.Props,
		"notify_props":         user.NotifyProps,
		"last_password_update": user.LastPasswordUpdate,
		"last_picture_update":  user.LastPictureUpdate,
		...
		"mfa_used_timestamps":  user.MfaUsedTimestamps,
	}
	if user.Email != "" {
		userMap["email"] = user.Email
	}

	return us.GetMaster().NamedExec(query, userMap)
}
...
func (us SqlUserStore) Update(rctx request.CTX, user *model.User, trustedUpdateData bool) (*model.UserUpdate, error) {
	...
	updateQuery := `UPDATE Users
			SET CreateAt=:create_at, UpdateAt=:update_at, DeleteAt=:delete_at, Username=:username, Password=:password,
				AuthData=:auth_data, AuthService=:auth_service, Email=:email, EmailVerified=:email_verified,
				Nickname=:nickname, FirstName=:first_name, LastName=:last_name, Position=:position, Roles=:roles,
				AllowMarketing=:allow_marketing, Props=:props, NotifyProps=:notify_props,
				LastPasswordUpdate=:last_password_update, LastPictureUpdate=:last_picture_update,
				FailedAttempts=:failed_attempts, Locale=:locale, Timezone=:timezone, MfaActive=:mfa_active,
				MfaSecret=:mfa_secret, RemoteId=:remote_id, LastLogin=:last_login, MfaUsedTimestamps=:mfa_used_timestamps
			WHERE Id=:id`

	user.Props = wrapBinaryParamStringMap(us.IsBinaryParamEnabled(), user.Props)

	// We use a temporary map for NamedExec to handle the NULL email, similar to insert,
	// so that saving a user with an empty email (like during deactivation clears/restorations)
	// won't trigger unique constraint violations.
	userMap := map[string]any{
		"id":                   user.Id,
		"create_at":            user.CreateAt,
		"update_at":            user.UpdateAt,
	...
	res, err := us.GetMaster().NamedExec(updateQuery, userMap)
	if err != nil {
		if IsUniqueConstraintError(err, []string{"Email", "users_email_key", "idx_users_email_unique"}) {
...
func applyViewRestrictionsFilter(query sq.SelectBuilder, restrictions *model.ViewUsersRestrictions, distinct bool) sq.SelectBuilder {
	...
	// FIXED (Issue #3 — RBAC Data Leakage):
	// The previous implementation used multiple JOINs which created an INTERSECTION (user MUST be in team X AND channel Y).
	// This was logically incorrect for "Community Manager + User Manager" roles where a user should see anyone
	// they share ANY boundary with (UNION).
	//
	// We now use a UNION of permissions using subqueries, which correctly implements the logic
	// and is significantly more performant as it avoids cross-product joins.
	//
	// We use sq.Expr with explicit subqueries to ensure correct SQL generation and slice expansion
...
func applyUserReportFilter(query sq.SelectBuilder, filter *model.UserReportOptions) sq.SelectBuilder {
	...
	query = applyViewRestrictionsFilter(query, filter.ViewRestrictions, true)
...
// DeactivateInactiveUsers deactivates all non-exempt users whose last activity
// predates the given threshold timestamp (milliseconds since epoch).
//
// Performance architecture (Issue #7 — Full Table Scan elimination):
//
// The previous implementation used two SQL anti-patterns that prevented index usage:
//  1. `Roles NOT LIKE '%system_admin%'`  — leading wildcard = full column scan on every row.
//  2. `Id NOT IN (SELECT UserId FROM Bots)` — correlated subquery evaluated per row.
//
// This implementation uses a two-phase strategy instead:
//
//	Phase 1 — Pre-compute exempt IDs (small, stable set, runs once):
//	  - Admins: `SELECT Id FROM Users WHERE Roles LIKE 'system_admin%'`
//	    Uses a trailing wildcard which CAN use a B-tree index on Roles (if present),
//	    and the admin set is typically < 10 rows regardless.
//	  - Bots: `SELECT UserId FROM Bots` — entire Bots table read once (typically < 50 rows).
//	  Both are combined via a UNION into a single []string of exempt IDs.
//
//	Phase 2 — Efficient bulk UPDATE:
//	  - `NOT IN (<exempt_ids>)` on the `Id` column (Primary Key).
//	    The query planner uses a hash anti-join or PK index lookup — O(k) not O(n).
//	  - `Id IN (SELECT UserId FROM Status WHERE LastActivityAt < ?)` uses the new
//	    index on Status(LastActivityAt) added in migration 000155.
//	  - `DeleteAt = 0` uses the existing index on DeleteAt.
//
// Result: query plan changes from O(n) full-scan to O(k) PK lookups,
// where k = len(exemptIds) ≈ constant regardless of total user count.
func (us SqlUserStore) DeactivateInactiveUsers(threshold int64) (int64, error) {
	curTime := model.GetMillis()

	// ── Phase 1: Pre-compute exempt IDs ─────────────────────────────────────────
	// Build a raw SQL query to fetch all exempt users in a single round-trip.
	// We use a UNION of two cheap queries:
	//   a) Admins: trailing-wildcard LIKE is sargable (can use index on Roles).
	//   b) Bots: full Bots table scan is cheap — the table is tiny by design.
	//
	// Note: 'system_admin' role is always the sole or leading role in the Roles
	// column (e.g. "system_admin" or "system_admin system_user"). A trailing wildcard
	// correctly captures all variants without a full column scan.
	exemptQuery := `
	...
	if err := us.GetReplica().Select(&exemptIDs, exemptQuery, "system_admin%"); err != nil {
		return 0, errors.Wrap(err, "failed to fetch exempt user IDs for inactive deactivation")
	}

	// ── Phase 2: Efficient bulk UPDATE ──────────────────────────────────────────
	// Build the UPDATE using squirrel so parameter binding is handled correctly.
	updateQuery := us.getQueryBuilder().
		Update("Users").
		Set("UpdateAt", curTime).
		Set("DeleteAt", curTime).
		Where(sq.Eq{"DeleteAt": 0}).
		Where(sq.Expr("Id IN (SELECT UserId FROM Status WHERE LastActivityAt < ?)", threshold))

	// Only add the NOT IN clause when there are exempt users to exclude.
	// An empty NOT IN list is always true — safe to skip for a tiny performance gain.
	if len(exemptIDs) > 0 {
		updateQuery = updateQuery.Where(sq.NotEq{"Id": exemptIDs})
	}

	result, err := us.GetMaster().ExecBuilder(updateQuery)
	if err != nil {
		return 0, errors.Wrap(err, "failed to deactivate inactive users")
	}

	count, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "failed to get rows affected for inactive user deactivation")
	}
