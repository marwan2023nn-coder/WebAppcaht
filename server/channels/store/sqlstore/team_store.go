...
type SqlTeamStore struct {
	*SqlStore
	...
	adminTeamIdsCache sync.Map // map[userID string][]string — keyed by userId
}
...
func (s SqlTeamStore) teamSearchQuery(opts *model.TeamSearch, countQuery bool) sq.SelectBuilder {
	...
	if len(opts.TeamIds) > 0 {
		query = query.Where(sq.Eq{"t.Id": opts.TeamIds})
	}
...
func (s SqlTeamStore) GetAllPage(offset int, limit int, opts *model.TeamSearch) ([]*model.Team, error) {
	...
	if opts != nil {
		...
		if len(opts.TeamIds) > 0 {
			builder = builder.Where(sq.Eq{"Teams.Id": opts.TeamIds})
		}
	}
...
func (s SqlTeamStore) AnalyticsTeamCount(opts *model.TeamSearch) (int64, error) {
	...
	if opts != nil && len(opts.TeamIds) > 0 {
		query = query.Where(sq.Eq{"Id": opts.TeamIds})
	}
...
// ClearCaches clears the in-process admin team IDs cache.
func (s *SqlTeamStore) ClearCaches() {
	s.adminTeamIdsCache.Range(func(key, _ any) bool {
		s.adminTeamIdsCache.Delete(key)
		return true
	})
}

// InvalidateAllTeamIdsForUser removes the cached admin team list for a specific user.
// Called whenever team membership changes so the next request re-fetches from DB.
//
//nolint:unparam
func (s *SqlTeamStore) InvalidateAllTeamIdsForUser(userId string) {
	s.adminTeamIdsCache.Delete(userId)
}
...
// GetAdminTeamIdsForUser returns the team IDs where the user is a Team Admin (SchemeAdmin = true).
// When allowFromCache is true, results are served from a process-level sync.Map to avoid
// repeating the same SQL query across multiple handlers in a single request lifecycle.
// The cache is invalidated via InvalidateAllTeamIdsForUser whenever team membership changes.
//
//nolint:unparam
func (s *SqlTeamStore) GetAdminTeamIdsForUser(userId string, allowFromCache bool) ([]string, error) {
	if allowFromCache {
		if cached, ok := s.adminTeamIdsCache.Load(userId); ok {
	...
	err = s.GetReplica().Select(&teamIds, query, args...)
	if err != nil {
		return []string{}, errors.Wrapf(err, "failed to find admin TeamMembers with userId=%s", userId)
	}

	if allowFromCache {
		s.adminTeamIdsCache.Store(userId, teamIds)
	}
	return teamIds, nil
}
