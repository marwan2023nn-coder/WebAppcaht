...
func (s *SqlGroupStore) GetGroups(page, perPage int, opts model.GroupSearchOpts, viewRestrictions *model.ViewUsersRestrictions) ([]*model.Group, error) {
	...
	groupsQuery = applyGroupViewRestrictionsFilter(groupsQuery, viewRestrictions)
...
func (s *SqlGroupStore) GroupCount(opts model.GroupSearchOpts, viewRestrictions *model.ViewUsersRestrictions) (int64, error) {
	countQuery := s.getQueryBuilder().
		Select("COUNT(*)").
		From("UserGroups")
	...
	if opts.Source != "" {
		countQuery = countQuery.Where("UserGroups.Source = ?", opts.Source)
	}

	if !opts.IncludeArchived && opts.Since <= 0 {
		countQuery = countQuery.Where("UserGroups.DeleteAt = 0")
	}

	var count int64
	if err := s.GetReplica().GetBuilder(&count, countQuery); err != nil {
		return int64(0), errors.Wrap(err, "failed to count Groups")
	}

	return count, nil
}

func applyGroupViewRestrictionsFilter(query sq.SelectBuilder, restrictions *model.ViewUsersRestrictions) sq.SelectBuilder {
	if restrictions == nil {
		return query
	}
