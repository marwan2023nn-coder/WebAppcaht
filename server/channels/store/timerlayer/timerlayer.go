...
func (s *TimerLayerFileInfoStore) CountAttached() (int64, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("FileInfoStore.CountAttached", success, elapsed)
	}
	return result, err
}

func (s *TimerLayerFileInfoStore) CountDownloaded() (int64, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("FileInfoStore.CountDownloaded", success, elapsed)
	}
	return result, err
}
...
func (s *TimerLayerFileInfoStore) IncrementDownloadCount(fileId string) error {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("FileInfoStore.IncrementDownloadCount", success, elapsed)
	}
	return err
}
...
func (s *TimerLayerGroupStore) GroupCount(opts model.GroupSearchOpts, viewRestrictions *model.ViewUsersRestrictions) (int64, error) {
	start := time.Now()
...
func (s *TimerLayerLinkMetadataStore) GetBulk(hashes []int64) ([]*model.LinkMetadata, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("LinkMetadataStore.GetBulk", success, elapsed)
	}
	return result, err
}
...
func (s *TimerLayerNotifyAdminStore) GetDataByUserIdAndFeature(userID string, feature model.SofaFeature) ([]*model.NotifyAdminData, error) {
	start := time.Now()
...
func (s *TimerLayerNotifyAdminStore) Update(userID string, requiredPlan string, requiredFeature model.SofaFeature, now int64) error {
	start := time.Now()
...
func (s *TimerLayerTeamStore) GetAdminTeamIdsForUser(userID string, allowFromCache bool) ([]string, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("TeamStore.GetAdminTeamIdsForUser", success, elapsed)
	}
	return result, err
}
