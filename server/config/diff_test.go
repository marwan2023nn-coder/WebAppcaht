...
func TestDiff(t *testing.T) {
	tcs := []struct {
		...
		},
		{
			...
			func() *model.Config {
				cfg := defaultConfigGen()
				cfg.PluginSettings.PluginStates[model.PluginIdNPS] = &model.PluginState{
					Enable: !cfg.PluginSettings.PluginStates[model.PluginIdNPS].Enable,
				}
				return cfg
			}(),
			ConfigDiffs{
				{
					...
					ActualVal: map[string]*model.PluginState{
						model.PluginIdNPS: {
							Enable: !defaultConfigGen().PluginSettings.PluginStates[model.PluginIdNPS].Enable,
						},
						model.PluginIdCalls: {
							Enable: true,
						},
